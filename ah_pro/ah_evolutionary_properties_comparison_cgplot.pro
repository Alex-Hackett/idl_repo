...
;
;-
PRO ah_evolutionary_properties_comparison_cgplot, TOSAVE=tosave, __savefile_dir_=__savefile_dir_
  ...
  P5 = LIST()
  bgcol = 'white'
;  CGPLOT, [0,0], [0,0], BACKGROUND=cgColor(bgcol), /WINDOW
;  CGPLOT, [0,0], [0,0], BACKGROUND=cgColor(bgcol), /WINDOW
;  CGPLOT, [0,0], [0,0], BACKGROUND=cgColor(bgcol), /WINDOW
;  CGPLOT, [0,0], [0,0], BACKGROUND=cgColor(bgcol), /WINDOW
;  CGPLOT, [0,0], [0,0], BACKGROUND=cgColor(bgcol), /WINDOW
...
cgPlot, [0,0], [0,0], /AddCmd
  ...
  FOREACH _savFileName_, _savFileList_, _index_ DO BEGIN
    ...
    cgPlotS, TeffArray, LogLsunArray, /AddCmd, XTITLE='Log $T_{eff}$', YTITLE = 'Log L / $L_{\odot}$' 
    ;###############################################
;    ;Plot0, HRD
;    HRDTITLE = STRTRIM('HR Diagram', 2)
;    HRDName = STRTRIM(STRING(modelname), 2)
...
;      OVERPLOT = P0[-1], NAME = HRDName, COLOR = colorFactor, BUFFER = buffervar, THICK = 2)
;
...
;      YTITLE = 'Log $T_{c}$ ($K$)', TITLE = CPLOTTITLE, OVERPLOT = P1[-1], NAME = CPLOTNAME, COLOR = colorFactor, BUFFER = buffervar, THICK = 2)
;
...
;      NAME = COREXNAME, COLOR = colorFactor, BUFFER = buffervar, THICK = 2, XRANGE = [1,0])
;
...
;      NAME = CORETNAME, COLOR = colorFactor, BUFFER = buffervar, THICK = 2)
;
...
;      NAME = EDDXNAME, COLOR = colorFactor, BUFFER = buffervar, THICK = 2, XRANGE = [1,0])
...
;      YTITLE = 'Log g ($cm/s^{2}$)', TITLE = LOGGTITLE, OVERPLOT = P5[-1], NAME = LOGGNAME, COLOR = colorFactor, BUFFER = buffervar, THICK = 2)

    ;Plotting Completed
  ENDFOREACH
  cgControl, Execute = 1
  cgControl, Output='/home/AHACKETT_Project/_PopIIIProject/IDL_GENERAL_COMPARISON_PLOTS/cgplotstest.pdf'
  ;Construct Legends
