;;PRO ZE_ETACAR_WORK_OPACITY_FOR_TOM_V2
...
fileop_all='/Users/jgroh/etacar_john_mod111_chi_op_au_43_54_64_80_125_164_216_354.txt'          ;in cm-1
fileeta_all='/Users/jgroh/etacar_john_mod111_eta_emiss_au_43_54_64_80_125_164_216_354.txt'      ;in erg/s/cm3/Hz
nd=65
...
;wavelengths vector:
wavelenght=[4360.,5448.,6400.,8000.,12500.,16400.,21600.,35400.]
...
Result = POLY_FIT(alog10(dist_cm[a:b]),REFORM(kappa_cm2_g[6,a:b]), 1,yfit=yfit_kappa)
...
;lineplot,dist_cm[a:b],10^yfit_kappa
 ...
 ynodes=xnodes
line_norm,alog10(dist_cm[a:b]),REFORM(kappa_cm2_g[6,a:b]),ynorm,norm,xnodes,ynodes
...
;Result_fit_interval = POLY_FIT(alog10(dist_cm[index_vec[i]:index_vec[i+1]-1]),REFORM(kappa_cm2_g[6,index_vec[i]:index_vec[i+1]-1]), norder,yfit=yfit_kappa_interval)
Result_fit_interval = SVDFIT(alog10(dist_cm[index_vec[i]:index_vec[i+1]-1]),REFORM(kappa_cm2_g[6,index_vec[i]:index_vec[i+1]-1]), norder+1,yfit=yfit_kappa_interval)
result_vec[*,i]=result_fit_interval
print,yfit_kappa_interval,REFORM(kappa_cm2_g[6,index_vec[i]:index_vec[i+1]-1])
...
;fitting K-band source function
...
print,i,index_vec[i],index_vec[i+1]-1
;Result_fit_interval = POLY_FIT(alog10(dist_cm[index_vec[i]:index_vec[i+1]-1]),REFORM(kappa_cm2_g[6,index_vec[i]:index_vec[i+1]-1]), norder,yfit=yfit_kappa_interval)
Result_fit_interval = SVDFIT(alog10(dist_cm[index_vec[i]:index_vec[i+1]-1]),REFORM(srce[6,index_vec[i]:index_vec[i+1]-1]), norder+1,yfit=yfit_srce_interval)
result_vec[*,i]=result_fit_interval
print,yfit_srce_interval,REFORM(srce[6,index_vec[i]:index_vec[i+1]-1])
IF I EQ 0 THEN yfit_vec=[yfit_srce_interval] ELSE yfit_vec=[yfit_vec,yfit_srce_interval]
print,yfit_vec
ENDFOR

lineplot,dist_cm[a:b],REFORM(10^srce[6,a:b])
lineplot,dist_cm[index_vec[0]:index_vec[nxnodes-1]-1],10^yfit_vec
