;PRO ZE_MDOTVAR_V7_TIMEDEP_ARMAGH_WORK_FLOWTIMESCALE_48
...
;uses as hydro structure model timedep_armagh/441
;teff =13.0 kK, mdot=2e-05 Msun/yr
;starting on 42 we are looking at variations as a function of deltat
;here deltat=240 days
...
READCOL,'/Users/jgroh/ze_models/timedep_armagh/'+modelin+'/RVSIG_COL_NEW_ND75',r1,v1,sigma1,depth1,COMMENT='!',F='D,D'

;r and v values of model 2
READCOL,'/Users/jgroh/ze_models/timedep_armagh/'+modelin+'/RVSIG_COL_NEW_ND75_V70',r2,v2,sigma2,depth2,COMMENT='!',F='D,D'
...
IF KEYWORD_SET(OLDV) THEN BEGIN
  for k=0, i-1 do begin
  if flowtime[k] lt deltat then vout[k]=v2[k] ELSE vout[k]=(10+(vinf[k]-10)*((1-(rstart[k]/r2[k]))^betaout))/(1+(10/0.04)*exp((rstart[k]-r2[k])/(0.04*rstart[k])))
  endfor 
ENDIF ELSE BEGIN
  for k=0, i-1 do begin
...
print,'Suggested Vmax: ', vout[index]
print,'Suggested Vmin: ', vout[index+1]
