PRO ZE_EVOL_PLOT_XY_GENERAL_EPS_V2,x,y,xtitle,ytitle,label,$
  z1=z1,labelz=labelz,ct=ct,minz=minz,maxz=maxz,$
  x2=x2,y2=y2,x3=x3,y3=y3,x4=x4,y4=y4,x5=x5,y5=y5,$
  x6=x6,y6=y6,x7=x7,y7=y7,x8=x8,y8=y8,x9=x9,y9=y9,x10=x10,y10=y10,x11=x11,y11=y11,x12=x12,y12=y12,x13=x13,y13=y13,$
  z2=z2, z3=z3, z4=z4, z5=z5, z6=z6, z7=z7, z8=z8, z9=z9,z_10=z_10,z_11=z_11,z_12=z_12,z_13=z_13,$
  ...
  pointsx7=pointsx7,pointsy7=pointsy7,psympoints7=psympoints7,symcolorpoints7=symcolorpoints7,symsizepoints7=symsizepoints7,$
  ...
  info=info,color1=color1,color2=color2,color3=color3,color4=color4,color5=color5,color6=color6,color7=color7,color8=color8,color9=color9,color_10=color_10,color_11=color_11,color_12=color_12,color_13=color_13,$
  linestyle1=linestyle1,linestyle2=linestyle2,linestyle3=linestyle3,linestyle4=linestyle4,linestyle5=linestyle5,linestyle6=linestyle6,linestyle7=linestyle7,linestyle8=linestyle8,linestyle9=linestyle9,linestyle_10=linestyle_10,linestyle_11=linestyle_11,linestyle_12=linestyle_12,linestyle_13=linestyle_13,$
  ...
  alt_pointsx2=alt_pointsx2,alt_pointsy2=alt_pointsy2,alt_psympoints2=alt_psympoints2,alt_symcolorpoints2=alt_symcolorpoints2,alt_symsizepoints2=alt_symsizepoints2,$
  alt_pointsx3=alt_pointsx3,alt_pointsy3=alt_pointsy3,alt_psympoints3=alt_psympoints3,alt_symsizepoints3=alt_symsizepoints3,alt_symcolorpoints3=alt_symcolorpoints3,$
  ...
  alt_factor=alt_factor,alt_plot_last=alt_plot_last ,alty_tickformat=alty_tickformat,$
  ...
  xcharsize=xcharsize,ycharsize=ycharsize,pcharsize=pcharsize,ticklen=ticklen,cmyk=cmyk,tempdir=tempdir
  ;working on supporting multiple vectors on the same plot. right now for vector with the same number of points.
  ;working OK up to 12 vectors (lines), but only 5  vectors for symbol
  ;use vectors 11 and 12 to plot data withtout z color coding in a plot where the 1st dataset is colorcoded and z1 is present
  ;implemented errorbars
  ;implemented alternate y axis
  close,/all
  !P.MULTI=0
  ...
  if n_elements(pcharsize) eq 0 then pcharsize=1.7
  ...
  if n_elements(ticklen) eq 0 then ticklen=15;*psysize/psxsize
  ...
  IF n_elements(fact_13) lt 1 THEN fact_13=factor
  ...
  if n_elements(cmyk) eq 0 then cmyk=0
  ...
  IF KEYWORD_SET(REBIN) THEN BEGIN
    ...
    IF ((N_elements(x13) GT 1) AND (N_elements(y13) GT 1)) THEN ZE_EVOL_REBIN_XYZ,x13,y13,x13r,y13r,z=z_13,rebin_z=rebin_z13,factor=fact_13
  ...
  if N_elements(rebin_z) lt 1  THEN BEGIN
    filename_prefix=tempdir+'/output_evol_'
  ...
  If n_elements(filename) GT 0 THEN psfilename=tempdir+filename+'.eps'

  device,filename=psfilename,/encapsulated,/color,bit=8,xsize=ps_xsize,ysize=ps_ysize,/inches,cmyk=cmyk
  ...
  if N_elements(z1) lt 1  THEN BEGIN ;no Z values to do color-coding
    if keyword_set(rebin) THEN BEGIN;datah as been rebinned
      ...
      IF ((N_elements(x13r) GT 1) AND (N_elements(y13r) GT 1)) THEN cgplots,x13r,y13r,noclip=0,color=color_13,linestyle=linestyle_13
      ;       IF KEYWORD_SET(DOUBLE_YAXIS) THEN BEGIN
      ;       ;   cgAXIS,YAXIS=1,YLOG=0,YRANGE=[alty_min,alty_max],YSTYLE=2,YTITLE='',/SAVE,color='black'
      ;          cgAXIS,YAXIS=1,YLOG=0,YRANGE=[alty_min,alty_max],YSTYLE=2,YTITLE=ALT_YTITLE,/SAVE,color='black'
    ...
    ENDIF ELSE BEGIN; data has not been rebinned
      ...
      IF ((N_elements(x13) GT 1) AND (N_elements(y13) GT 1)) THEN cgplots,x13,y13,noclip=0,color=color_13,linestyle=linestyle_13,psym=psym_13,symsize=symsize_13,symcolor=symcolor_13
    ENDELSE
  ENDIF ELSE BEGIN ;doing color coding, but has to do to all of them
    IF keyword_set(rebin) THEN BEGIN;datah as been rebinned
      ...
      IF ((N_elements(x13) GT 1) AND (N_elements(y13) GT 1)) THEN cgplots,x13r,y13r,noclip=0,color=bytscl(rebin_z13,MIN=minz,MAX=maxz),linestyle=linestyle_13,psym=psym_13  ,symsize=symsize_13
    ...
    ENDIF else begin ;data has not been rebinned
      ...
      IF ((N_elements(x13) GT 1) AND (N_elements(y13) GT 1)) THEN cgplots,x13,y13,noclip=0,color=color_13,linestyle=linestyle_13,psym=psym_13  ,symsize=symsize_13
  ...
  IF ((N_elements(pointsx7) GT 1) AND (N_elements(pointsy7) GT 1)) THEN cgplots,pointsx7,pointsy7,psym=psympoints7,symcolor=symcolorpoints7,symsize=symsizepoints7,noclip=0
  ...
  IF KEYWORD_SET(NOLABEL) THEN print,'Not putting labels on variables' ELSE BEGIN
    ...
    IF (N_elements(labeldatapoints2) GT 1) THEN FOR i=0, n_elements(labeldatapoints5)-1 DO xyouts,pointsx6[i],pointsy6[i],strcompress(STRING(labeldatapoints6[i])),charsize=1.5,color=fsc_color('black'),/DATA,noclip=0
    IF (N_elements(labeldatapoints2) GT 1) THEN FOR i=0, n_elements(labeldatapoints5)-1 DO xyouts,pointsx7[i],pointsy7[i],strcompress(STRING(labeldatapoints7[i])),charsize=1.5,color=fsc_color('black'),/DATA,noclip=0
  ENDelse

  ;assumes alt data has been rebinned, no color coding with z values HAVE TO WORK HERE TO BE MORE GENERAL
  IF KEYWORD_SET(DOUBLE_YAXIS) THEN BEGIN
    ;   cgAXIS,YAXIS=1,YLOG=0,YRANGE=[alty_min,alty_max],YSTYLE=2,YTITLE='',/SAVE,color='black'
    cgAXIS,YAXIS=1,YLOG=0,YRANGE=[alty_min,alty_max],YSTYLE=1,YTITLE=ALT_YTITLE,/SAVE,color='black',YTICKFORMAT=alty_tickformat
    ;           print,'entrei', alt_x2,alt_y2
    IF ((N_elements(alt_x2) GT 1) AND (N_elements(alt_y2) GT 1)) THEN cgplots,alt_x2,alt_y2,color=alt_color2,noclip=0,psym=alt_psym2,symsize=alt_symsize2,symcolor=alt_symcolor2,linestyle=alt_linestyle2
    IF ((N_elements(alt_x3) GT 1) AND (N_elements(alt_y3) GT 1)) THEN cgplots,alt_x3,alt_y3,color=alt_color3,noclip=0,psym=alt_psym3,symsize=alt_symsize3,symcolor=alt_symcolor3,linestyle=alt_linestyle3
    IF ((N_elements(alt_x4) GT 1) AND (N_elements(alt_y4) GT 1)) THEN cgplots,alt_x4,alt_y4,color=alt_color4,noclip=0,linestyle=alt_linestyle4
    IF ((N_elements(alt_x5) GT 1) AND (N_elements(alt_y5) GT 1)) THEN cgplots,alt_x5,alt_y5,color=alt_color5,noclip=0,linestyle=alt_linestyle5
    IF ((N_elements(alt_x6) GT 1) AND (N_elements(alt_y6) GT 1)) THEN cgplots,alt_x6,alt_y6,color=alt_color6,noclip=0,linestyle=alt_linestyle6
    IF ((N_elements(alt_x7) GT 1) AND (N_elements(alt_y7) GT 1)) THEN cgplots,alt_x7,alt_y7,color=alt_color7,noclip=0,linestyle=alt_linestyle7
    IF ((N_elements(alt_x8r) GT 1) AND (N_elements(alt_y8r) GT 1)) THEN cgplots,alt_x8r,alt_y8r,color=alt_color8,noclip=0,linestyle=alt_linestyle8
    IF ((N_elements(alt_x9r) GT 1) AND (N_elements(alt_y9r) GT 1)) THEN cgplots,alt_x9r,alt_y9r,color=alt_color9,noclip=0,linestyle=alt_linestyle9
    IF ((N_elements(alt_x10r) GT 1) AND (N_elements(alt_y10r) GT 1)) THEN cgplots,alt_x10r,alt_y10r,color=alt_color_10,noclip=0 ,linestyle=alt_linestyle_10
    IF ((N_elements(alt_x11r) GT 1) AND (N_elements(alt_y11r) GT 1)) THEN cgplots,alt_x11r,alt_y11r,noclip=0,color=alt_color_11,linestyle=alt_linestyle_11
