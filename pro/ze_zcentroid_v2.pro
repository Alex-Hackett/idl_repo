...
;-
;-------------------------------------------------------------------------------
PRO ZE_ZCENTROID_V2,w,f,index,minx,maxx,WL,WR,XC,WC,AA
if n_params(0) eq 0 then $
  message,' Calling Sequence:  ZCENTROID, w, f, index, WL, WR, XC, WC, fwhm_ret'
...
;
  ;added by JHG in order to proceed even if no centroid is found (e.g. for low S/N data); sets the centroid to the central line of the range.
  IF MAX(WHERE(W LE ((AA(1)/c)+1)*(w0))) LT WL THEN AA(1)=0
  IF MIN(WHERE(W GE ((AA(1)/c)+1)*(w0))) LT WL THEN AA(1)=0
...
;                                                            
;                                                            
  wc=((AA(1)/c)+1)*(w0)  ;actual derived centroid wavelength ADDED BY JHG OTHERWISE DOES NOT WORK...
  
  ;fwhm_rec=((AA(0)/c)+1)*(w0)
  ;print,fwhm_rec
  ;still doing plots for consistency check ADDED BY JHG
;  PLOT,W,10*(F/FSCALE),xstyle=1,/NOERASE, $
;    xmargin=[12,6],ymargin=[4,15],psym=0,xtitle='Wavelength(A)', $
;  title='FP_MERGE ( using CENTROID option)'
 oplot,w(xl(0):xr(0)),yfit,psym=0,thick=2,color=FSC_COLOR('red')
...
window_id=!D.WINDOW
PLOT,W,10*(F/FSCALE),xstyle=1,ystyle=1,yrange=[10.*min(F/FSCALE),10*max(F/FSCALE)],/NOERASE, $
    xmargin=[12,6],ymargin=[4,15],psym=0,xtitle='Wavelength(A)'
;zooming in

window
PLOT,W,10*(F/FSCALE),xstyle=1,ystyle=1,$
    xmargin=[12,6],ymargin=[4,15],psym=0,xtitle='Wavelength(A)',xrange=[w(minx),w(maxx)],yrange=[min(10*(F[minx:maxx]/FSCALE)),max(10*(F[minx:maxx]/FSCALE))]
new_window_id=!D.WINDOW
...
if !d.name eq 'TEK' or !d.name eq 'VGA' then begin
  ...
  OPLOT,XVECT(2:3),!y.crange,linestyle=1
    ...
    OPLOT,XVECT(0:1),!y.crange,linestyle=1
    wset,window_id
    PLOTS,XVECT(0:1),!y.crange,linestyle=1
    wset,new_window_id
