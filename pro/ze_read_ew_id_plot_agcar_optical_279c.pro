...
obsfuse='/home/groh/espectros/agcar/agc02jul04_36a92n.txt'  
...
;defines model file
model='/home/groh/espectros/agcar/279_narv2_ed.txt'  
...
max1=1.5
plot,lambdafuse,alog10(fluxfuse),xstyle=1,ystyle=1,xrange=[x1l,x1u],yrange=[-0.2,max1],xtitle='wavelength (Angstrom)',$
ytitle='Log Normalized Flux',/nodata,charsize=2
plots,lambdafuse,alog10(fluxfuse),color=fsc_color('black'),noclip=0,clip=[x1l,-0.2,x1u,max1]
plots,lambdamod,alog10(fluxmod),color=fsc_color('red',!d.table_size-10), noclip=0,clip=[x1l,-0.2,x1u,max1],linestyle=1,thick=3.5
...
;2nd panel
max2=6.5
plot,lambdafuse,fluxfuse,xstyle=1,ystyle=1,xrange=[x2l,x2u],yrange=[0,max2],xtitle='wavelength (Angstrom)', ytitle='Normalized Flux',/nodata,charsize=2
plots,lambdafuse,fluxfuse,color=fsc_color('black'),noclip=0,clip=[x2l,0,x2u,max2]
plots,lambdamod,fluxmod,color=fsc_color('red'), noclip=0,clip=[x2l,0,x2u,max2],linestyle=1,thick=3.5
for l=0, i-1 do begin
if (lambdac[l] gt x2l) and (lambdac[l] lt x2u) and (abs(ew[l]) gt 0.30) then begin
pos=strpos(strtrim(sob[l],1),'(')
sobc[l]=strmid(sob[l],4,pos)
if ((sobc[l] ne 'NII') && (sobc[l] ne 'HI')) then begin ;colocando apenas um traco nas transicoes do FeV,normal para os outros.
xyouts,lambdac[l]+6,max2-1.2,'!3'+string(45B)+sobc[l],alignment=0.5,orientation=90
endif
if (sobc[l] eq 'NII') then begin
xyouts,lambdac[l]+6,max2-1.8,'!3'+string(45B)+sobc[l],alignment=0.5,orientation=90
endif
...
;3rd panel
max3=3.0
plot,lambdafuse,fluxfuse,xstyle=1,ystyle=1,xrange=[x3l,x3u],yrange=[0.7,max3],xtitle='wavelength (Angstrom)', ytitle='Normalized Flux',/nodata,charsize=2
plots,lambdafuse,fluxfuse,color=fsc_color('black'), noclip=0,clip=[x3l,0.7,x3u,max3]
plots,lambdamod,fluxmod,color=fsc_color('red'), noclip=0,clip=[x3l,0.7,x3u,max3],linestyle=1,thick=3.5
for l=0, i-1 do begin
if (lambdac[l] gt x3l) and (lambdac[l] lt x3u) and (abs(ew[l]) gt 0.1) then begin
pos=strpos(strtrim(sob[l],1),'(')
sobc[l]=strmid(sob[l],4,pos)
if ((sobc[l] ne 'OI') && (sobc[l] ne 'FeVI')) then begin ;colocando apenas um traco nas transicoes do FeV,normal para os outros.
xyouts,lambdac[l]+4,max3-0.8,'!3'+string(45B)+sobc[l],alignment=0.5,orientation=90
endif
if (sobc[l] eq 'OI') then begin
xyouts,lambdac[l]+4,max3-0.25,'!3'+string(45B)+sobc[l],alignment=0.5,orientation=90
endif
if(sobc[l] eq 'FeVI') then begin
xyouts,lambdac[l],5,'!3'+string(45B),alignment=0.5,orientation=90
endif
