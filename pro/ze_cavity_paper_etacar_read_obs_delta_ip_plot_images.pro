...
PRO ZE_COMPUTE_IMAGE_I_P_DELTA_LAMBDA_V2,pa,rstar,dist,lambda_val,lambda0,lambda,np,ndelta1,nos,ipdelta,p,delta1_vec,x,y,circ_GridSpace_mas,intens2d_lambda=intens2d_lambda,$
circxvector=circxvector,circyvector=circyvector,circ_griddedData=circ_griddedData,norm=norm,pip=pip
...
circxvector,circyvector,circ_griddedData,imginv=imginv,VEL=vel,Norm=Norm,log=log
...
;sufix='99_45_noscale'
model2d='cut_v4_var_inc_nowall'
;sufix='i00'
model2d='azim_changes_test'
;sufix='1d6em3'
;sufix='a8'
;sufix='latidep6'
...
;sufix='1384_falpha10'
;sufix='5506_falpha10'
;sufix='21029_falpha10'


;sufix='halpha_falpha10'
;sufix='halpha_falpha10_scl'
;sufix='halpha'
;sufix='halpha_nowall'
;sufix='halpha_alpha57_falpha4_scl_delta81' 
;sufix='halpha_alpha57_falpha4_scl_i311' 
;sufix='fe2_5318_n2_5755'
;sufix='halpha_ndelta81'
...
pa=312.          ;position angle in degrees; by definition PA=0 towards N, and increase eastwards. USE PA significantly different than 0 (e.g.50) for spherical models
pa=220.
;pa=0.
...
;using new inclination found by tom?

inc=180-(90-result) ;remember that for cavity models inclination=90 - ANG1
inc_str=strcompress(string(inc, format='(I03)'))
...
lambda_ini=1387.7
lambda_fin=1388.13
lambda_sampling=0.04     ;sampling in Angstrom ; use 1
END
...
'12617_falpha10': BEGIN
lambda_ini=12620.
lambda_fin=12680.
lambda_sampling=10.8     ;sampling in Angstrom ; use 1
END

'16569_falpha10': BEGIN
lambda_ini=16570.
lambda_fin=16620.
lambda_sampling=7.8     ;sampling in Angstrom ; use 1
END

'21029_falpha10': BEGIN
lambda_ini=21040.
lambda_fin=21060.
lambda_sampling=4.8     ;sampling in Angstrom ; use 1
END

'35556_falpha10': BEGIN
lambda_ini=35560.
lambda_fin=35610.
lambda_sampling=5.8     ;sampling in Angstrom ; use 1
END
...
'halpha_falpha10': BEGIN
lambda0=6564.61
lambda_ini=6550.
lambda_fin=6580.
lambda_sampling=0.2     ;sampling in Angstrom ; use 1
END

'halpha_falpha10_scl': BEGIN
lambda0=6564.61
lambda_ini=6550.
lambda_fin=6580.
lambda_sampling=0.2     ;sampling in Angstrom ; use 1
...
'halpha_alpha57_falpha4_scl_delta81': BEGIN
lambda0=6564.61
lambda_ini=6550.
lambda_fin=6580.
lambda_sampling=0.2     ;sampling in Angstrom ; use 1
...
'halpha_alpha57_falpha4_scl_i311': BEGIN
lambda0=6564.61
lambda_ini=6550.
lambda_fin=6580.
lambda_sampling=0.2     ;sampling in Angstrom ; use 1
...
'fe2_5318_n2_5755': BEGIN
lambda0=5318.0944 ;for Fe II 5318
lambda_ini=5310.
lambda_fin=5330.

;lambda0=5756.240 ;for [NII] 5756 
;lambda_ini=5745.
;lambda_fin=5765.
;lambda_fin=5752.0
lambda_sampling=0.2     ;sampling in Angstrom ; use 1
...
circ_GridSpace_mas=[0.010,0.010];best for continuum images, 5 mas
;circ_GridSpace_mas=[0.035*1.4,0.035*1.4];best so far for Halpha, 20 mas
;circ_GridSpace_mas=[0.035*4.4,0.035*4.4]
;circ_GridSpace_mas=[0.035*14.,0.035*14.0];200 mas
;circ_GridSpace_mas=[0.035*70.,0.035*70.0];1000 mas
;circ_GridSpace_mas=[0.035*0.5,0.035*0.5];test for azim changes

;computes flux-like quantity p*I(p)? should NOT do that for havin an idea where flux comes from:
;DO NOT MULTIPLY BY P, since  we are doing the integral in P and delta (BH05 eq 12), F=1/d^2 *Int(0,2pi)Int(0,pmax)I(p,delta)dp ddelta, see BH05 eq. 12
PIP=0
key_norm=1
;computes continuum image -- assumes it corresponds to lambda_ini
print,lambda0,lambda_ini
ZE_COMPUTE_IMAGE_I_P_DELTA_LAMBDA_V2,pa,rstar,dist,lambda_ini,lambda0,lambda,np,ndelta1,nos,ipdelta,p,delta1_vec,x,y,circ_GridSpace_mas,intens2d_lambda=intens2d_lambda,$
circxvector=circxvector,circyvector=circyvector,circ_griddedData=circ_griddedData_cont,norm=key_norm,pip=pip
help,circ_griddedData_cont
circ_griddedData_cont[*,*]=0.

ZE_COMPUTE_IMAGE_I_P_DELTA_LAMBDA_V2,pa,rstar,dist,lambda0,lambda0,lambda,np,ndelta1,nos,ipdelta,p,delta1_vec,x,y,circ_GridSpace_mas,intens2d_lambda=intens2d_lambda,$
circxvector=circxvector,circyvector=circyvector,circ_griddedData=circ_griddedData_0,norm=key_norm,pip=pip

for i=0., nlambda-1 do begin
lambda_val=lambda_ini + i*lambda_sampling
lambda_vector[i]=lambda_val
ZE_COMPUTE_IMAGE_I_P_DELTA_LAMBDA_V2,pa,rstar,dist,lambda_val,lambda0,lambda,np,ndelta1,nos,ipdelta,p,delta1_vec,x,y,circ_GridSpace_mas,intens2d_lambda=intens2d_lambda,$
circxvector=circxvector,circyvector=circyvector,circ_griddedData=circ_griddedData,/NORM
...
dir_temp='/Users/jgroh/temp/'
file='/Users/jgroh/temp/latex_figure_img_'+model+'_'+model2d+'_'+sufix+'_d'+dstr+'_'+lambda_ini_str+'_'+lambda_fin_str+'.tex'
filenoext='/Users/jgroh/temp/latex_figure_img_'+model+'_'+model2d+'_'+sufix+'_d'+dstr+'_'+lambda_ini_str+'_'+lambda_fin_str
...
;for continuum
lambda_val=lambda0
ZE_ETACAR_CREATE_INTENSITYIMAGE_CONTINUUM_EPS,pa,rstar,dist,lambda_val,lambda0,lambda,np,ndelta1,nos,ipdelta,p,delta1_vec,x,y,model,model2d,sufix,dstr,inc_str,circ_GridSpace_mas,$
...
;;for halpha
ZE_ETACAR_CREATE_INTENSITYIMAGE_CONTSUB_EPS,circ_griddedData_cont,pa,rstar,dist,6556.6,lambda0,lambda,np,ndelta1,nos,ipdelta,p,delta1_vec,x,y,model,model2d,sufix,dstr,inc_str,circ_GridSpace_mas,$
                                            circ_griddedData=circ_griddedData,circxvector=circxvector,circyvector=circyvector,PIP=pip,sqrt=0,norm=1

ZE_ETACAR_CREATE_INTENSITYIMAGE_CONTSUB_EPS,circ_griddedData_cont,pa,rstar,dist,6564.61,lambda0,lambda,np,ndelta1,nos,ipdelta,p,delta1_vec,x,y,model,model2d,sufix,dstr,inc_str,circ_GridSpace_mas,$
                                            circ_griddedData=circ_griddedData,circxvector=circxvector,circyvector=circyvector,PIP=pip,sqrt=0,norm=1
;
;
ZE_ETACAR_CREATE_INTENSITYIMAGE_CONTSUB_EPS,circ_griddedData_cont,pa,rstar,dist,6570.61,lambda0,lambda,np,ndelta1,nos,ipdelta,p,delta1_vec,x,y,model,model2d,sufix,dstr,inc_str,circ_GridSpace_mas,$
                                            circ_griddedData=circ_griddedData,circxvector=circxvector,circyvector=circyvector,PIP=pip,sqrt=0,norm=1

;generate PDF?
;spawn,'/usr/texbin/latex -output-directory=/Users/jgroh/temp '+file ,/sh
;spawn,'/usr/texbin/dvipdfm -o '+filenoext+'.pdf '+filenoext+'.dvi' ,/sh

;above is not working in Lion, use:
output_dir='/Users/jgroh/temp'
CD, output_dir
;spawn,'/usr/texbin/simpdftex latex '+file,/sh


;open PDF?
;spawn,'open '+filenoext+'.pdf',/sh
