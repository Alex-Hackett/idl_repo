;PRO ZE_ETACAR_STIS_PLOT_ALL_OFFSETS_WITH_TED_Jun2009
;routine to create the interpolated mapping for the STIS data from Ted
;2010sep24

noffset=21
dir='/Users/jgroh/data/hst/stis/etacar_from_ted/Post_SM4/Jun_2009/results/'
...
;file=list[j+0] ;for 7281
file=list[j+21] ;for 4701
;file=list[j+42] ;for 5735
...
colu_up=470.
;normalize or subtract each row by the continuum? crudely done
for i=0, sizaxis2new - 1 do fluxsc1[*,i]=fluxsc1[*,i]-fluxsc1[colu,i]
;for i=0, sizaxis2new - 1 do fluxsc1[*,i]=fluxsc1[*,i]/fluxsc1[colu,i]

fluxcr=fluxsc1[index1:index2,*] ;cropped vector in spectral and spatial direction 
;rotate by -pa?
;fluxcr=ROT(fluxcr,pa)
sizaxis1new=index2-index1
dataa2_all_offsets_contsub_crop[*,*,j]=fluxcr
ENDFOR

;REVERSING THE offset along the slit direction ONLY!
;dataa2_all_offsets_contsub_crop=REVERSE(dataa2_all_offsets_contsub_crop,2)
dataa2_all_offsets_contsub_crop=REVERSE(dataa2_all_offsets_contsub_crop,3)
...
;;wset,19
;;file_output='/Users/jgroh/temp/etc_stis_mapping_jun2009_'+lambda0str+'_'+strcompress(string(lambda_val*1000, format='(I08)'))
;sequential file numbering has to be used to propely create animated GIFs using convert
;file_output='/Users/jgroh/temp/etc_stis_mapping_jun2009_'+line_for_filename+'_'+strcompress(string(index_lambda, format='(I04)'))
...
sufix_gif='etc_stis_mapping_jun2009_'+line_for_filename+'_'
movie_output_gif='etc_stis_mapping_jun2009_'+line_for_filename+'.gif'
movie_output_mov='etc_stis_mapping_jun2009_'+line_for_filename+'.mov'
digits=4
init_image=0
end_image=index2-index1
delay_time='30'
convert_mov=1 ; set to 0 to not convert; converting to quicktime .mov will only work in Mac OSX with QTAmateur installed;
...
;dataa2_all_offsets_contsub_crop[*,*,*]=1e-14
...
dataa2_velint=TOTAL(dataa2_all_offsets_contsub_crop[index7:index8,*,*],1)/(index8-index7+1)*(lambda_new[index8]-lambda_new[index7])
a=0.0
b=max(dataa2_velint)/10.
;b=4000.0
img_m500_m250_contsub=bytscl(dataa2_velint,MIN=a,MAX=b)

;dataa2_velint=ROT(dataa2_velint,-pa)
...
;integrate image fluxes
dataa2_velint2=dblarr(index4-index3+1,noffset)
dim1=index8-index7+1
dim2=index4-index3+1
dim3=noffset
for s=0, index4-index3 do begin
  for r=0, noffset-1 do begin
  print,s,r
  brightness_profile=dataa2_all_offsets_contsub_crop[index7:index8,s,r]
  dataa2_velint2[s,r]=int_tabulated(lambda_new[index7:index8],brightness_profile,/DOUBLE) 
  endfor
endfor

a2=0.0
b2=max(dataa2_velint2)/10.
;b=4000.0
img_m500_m250_contsub2=bytscl(dataa2_velint2,MIN=a2,MAX=b2)
ZE_HST_STIS_MAPPING_PLOT_MONOCHROMATIC_IMAGE_XWINDOW,img_m500_m250_contsub2,a2,b2,offset_along_slit_arc_cut,offset_vector,aa,bb,lambda_val,lambda0_micron,ct,nointerp

ratio=dataa2_velint/dataa2_velint2
a3=min(ratio)
b3=max(ratio)/1.
ct=13
a3=0.9
b3=1.3
;b=4000.0
ratioimage=bytscl(ratio,MIN=a3,MAX=b3)
ZE_HST_STIS_MAPPING_PLOT_MONOCHROMATIC_IMAGE_XWINDOW,ratioimage,a3,b3,offset_along_slit_arc_cut,offset_vector,aa,bb,lambda_val,lambda0_micron,ct,nointerp
