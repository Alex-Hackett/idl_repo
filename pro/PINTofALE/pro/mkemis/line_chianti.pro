...
for i=0,nion-1 do begin				;for each ion...
  ...
  if is_keyword_set(delec) then $
	print,'Ionic state (DR)',jon else $
  ...
  if nw gt 0 then begin			;(there are some lines
    ...
    splups=chline.spups
    for j=0,ntrans-1 do begin			;{for each line
	...
	;if maxups gt 0. then upstst=1 else upstst=0
      ...
      if total(pop[*,l2]) ne 0 then siz_chianti=siz_true
	...
	;------------------------------------------------------------;
        ...
        intens=dblarr(nt)
	if siz_chianti gt 0 then for k=0,nt-1 do intens[k]=hc*a_val*pop[k,l2]

        ;	append each transition to output(s)
        fx=[fx,intens] & ang=[ang,wvl] & ikey=[ikey,jon]
	jkey=[jkey,jon+delec] & kev=[kev,nrg]
	if not is_keyword_set(ow) then $
		ow=[long(j)] else ow=[ow,long(j)]
    ...
    ow=0L
    if not is_keyword_set(itrans) then begin	;first
