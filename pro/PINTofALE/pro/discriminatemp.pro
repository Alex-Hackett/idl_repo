...
if nerr gt 1 then fsig[0L:(nerr<nL)-1L]=ferr[0L:(nerr<nL)-1L]
if nerr eq 1 then begin
  if ferr[0] lt 1 then fsig[*]=ferr[0]
  if ferr[0] ge 1 and ferr[0] lt 100 then fsig[*]=ferr[0]/100.
  if ferr[0] ge 100 then fsig[*]=1./ferr[0]
endif
;
...
;	output
dtemp=bytarr(nT,nT)
outflx=fltarr(nT,nL) & eflx=outflx
dTmatrix=fltarr(nT,nT)
...
for i=0L,nT-1L do begin
  ...
  outflx[i,*]=fx & eflx[i,*]=efx
endfor

;	construct the discriminability matrix
for i=0L,nT-1L do for j=0L,nT-1L do dTmatrix[i,j]=total((outflx[i,*]-outflx[j,*])^2/(eflx[i,*]^2+eflx[j,*]^2),/nan)
