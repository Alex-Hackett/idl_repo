...
;
zEQFILE='ioneq/mazzotta_etal.ioneq'
...
for iz=0L,mz-1L do begin
  ...
  if zz(iz) le 0 then begin		;{error! error!
    ...
    if keyword_set(useuserarr) then ioneq=userarr(*,*,iz) else begin
	  ...
	  if not keyword_set(eqfile) then eqfile=zEQFILE ;'ioneq/arnaud_rothenflug.ioneq'
        endelse
        ;eqfile=chifil
        ;if szc(nszc-2) ne 7 then eqfile=zEQFILE ;'ioneq/arnaud_rothenflug.ioneq'
