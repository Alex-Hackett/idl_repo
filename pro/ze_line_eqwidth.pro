...
;
; CALLING SEQUENCE:
; ze_line_eqwidth,wave,flux,results
;
; INPUTS:
...
;-
;========================================================= EVENT DRIVER
pro ze_line_eqwidth_event,event

  common ze_line_eqwidth,auto_exit_val
  ...
  case uvalue of 
    ...
    ze_line_eqw_plot,(*info)
    widget_control,(*info).log2,set_v=' '
    end
      'PS': ze_line_eqw_ps,(*info)
      'WREST': begin
  ...
  if compute eq 1 then begin  
    ...
    ze_line_eqw_compute,info
    ze_line_eqw_plot,(*info)
    widget_control,(*info).ps,sensitive=1
    if auto_exit_val eq 1 THEN widget_control,event.id,set_uvalue='EXIT'
  end
...
; Main routine
;
pro ze_line_eqwidth,wave,flux,results,wrest=wrest,lineid=lineid,title=title, $
  group=group,modal=modal,wmin=wmin,wmax=wmax,fmin=fmin,fmax=fmax,auto_exit=auto_exit
  
  common ze_line_eqwidth,auto_exit_val

   IF KEYWORD_SET(auto_exit) THEN auto_Exit_val=1 ELSE auto_Exit_val=0
  ...
  if n_elements(fmin) eq 0 then fmin = min(flux[findel(wmin,wave):findel(wmax,wave)])
  if n_elements(fmax) eq 0 then fmax = max(flux[findel(wmin,wave):findel(wmax,wave)])
