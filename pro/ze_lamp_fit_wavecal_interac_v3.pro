;v3 allows for a previous fit to be read -- using when identifying weak lamp lines after the strongest lines have been identified NOT WOKRING SO FAR, USE v2
;v2 return fit parameters 
...
;
pro ze_lamp_fit_wavecal_interac_v3_event,event
  common ze_lamp_fit_wavecal_interac_v3_common,info,xarray,yarray,lamparray,params_array,fitvalues,rsave,gsave,bsave,xn,yn,xnorm,fnorm
...
;
    if (event.press eq 4) or $
     ((event.press eq 1) and (info.state eq 'DELETE')) then begin
      diff = abs(xn-v(0))
...
replot_spline:
  wset,info.plot1_id
  device,copy=[0,0,950,450,0,0,info.pixid]
  params_array=poly_fit(xn,lamparray,info.degree,yfit=fitvalues)
  xarray=xn
  yarray=lamparray - fitvalues
  yn=yarray
  line_norm_plot,info,xarray,yarray,xn,yn
  oplot,xn,yn,color=fsc_color('red'),symsize=1,psym=2,thick=2
  wset,info.pixid2
...
pro ze_lamp_fit_wavecal_interac_v3,line_centroids,lampline_ids,xnodes,ynodes,xcen,ycen,lambda_fit_params,lambda_fit_values,tharlineid, yfitold,$
  title=title,xtitle=xtitle,ytitle=ytitle, $
  group=group,xrange=xrange,yrange=yrange, $
  min_val=min_val,max_val=max_val, modal=modal,OLDFIT=OLDFIT
;
; CALLING SEQUENCE:
; 
...
;
  ...
  ns = n_elements(line_centroids)
  ;do initial fit with order 1 polynomial
  degree=1
  yfit=poly_fit(line_centroids,lampline_ids,degree,yfit=fit)
  xarray = line_centroids
  yarray = lampline_ids-fit
  lamparray=lampline_ids
  xnodes=line_centroids
  ynodes=lampline_ids-fit
  lambda_fit_params=yfit
  lambda_fit_values=fit
  
  IF KEYWORD_SET(OLDFIT) THEN BEGIN
    yfit=yfitold
    ZE_COMPUTE_LAMBDA_FROM_POLYFIT_PARAMS,ns,yfitold,lambda_old ;not WORKING
    yarray = lampline_ids-lambda_old
    ynodes=lampline_ids-lambda_old
  ENDIF
...
;
  ...
  lab = widget_label(b,value='Polynomial degree')
  slider = widget_slider(b,min=0,max=10,value=1, $
    /drag,uvalue='DEGREE',xsize=800,title='')
...
;
  ...
  info = {message:message,ns:ns,pixid:pixid,pixid2:pixid2,xtitle:xtitle, $
    ...
    min_val:min_val,max_val:max_val,degree:1}
;
; set initial range
; 
...
;
  ...
  line_norm_plot,info,xarray,yarray,xn,yn
  oplot,xn,yn,color=fsc_color('red'),symsize=1,psym=2,thick=2
  xmanager,'ze_lamp_fit_wavecal_interac_v3',main
  ...
  xcen=xn
  ycen=yn
  tharlineid=lamparray
  lambda_fit_params=params_array
  lambda_fit_values=fitvalues
