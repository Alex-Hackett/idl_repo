;PRO ZE_MDOTVAR_V7_TIMEDEP_ARMAGH_WORK_FLOWTIMESCALE
;trying to compute tflow by integrating the inverse of the velocity law
;based on model hrcar 6, trying ot converge a model with T circa 18kK
...
READCOL,'/Users/jgroh/ze_models/timedep_armagh/'+modelout+'/RVSIG_COL_NEW_v150',r1,v1,sigma1,depth1,COMMENT='!',F='D,D'

;r and v values of model 2
READCOL,'/Users/jgroh/ze_models/timedep_armagh/'+modelout+'/RVSIG_COL_NEW_v150',r2,v2,sigma2,depth2,COMMENT='!',F='D,D'

;;read r and v values from input model
;READCOL,'/Users/jgroh/ze_models/timedep_armagh/'+modelin+'/RVSIG_COL',r22,v22,sigma22,depth22,COMMENT='!'
...
;v2i=cspline(r2,v2,r2i)
;
;cut=MIN(ABS(v2i-vs),indexcut)
;v2icutrev=REVERSE(v2i[0:indexcut])
;r2icutrev=REVERSE(r2i[0:indexcut])
;
;dflowtimerev=dblarr(n_elements(r2icutrev))
;flowtimerev=dblarr(n_elements(r2icutrev))
;dflowtimerev[0]=0.0D
;flowtimerev[0]=0.0D
;S_TO_DAY=1.0d/(24.0D*3600.0D)
;FOR l=1.0D, n_elements(r2icutrev) - 1.0D DO BEGIN
;dflowtimerev[l]=S_TO_DAY*(r2icutrev[l]-r2icutrev[l-1])*1e5/v2icutrev[l]
;flowtimerev[l]=flowtimerev[l-1]+dflowtimerev[l]
;ENDFOR
...
;
;rrev[i2+1:i3]=r1[i2+1:i3]
;vrev[i2+1:i3]=v1[i2+1:i3]
;;
...
;
;;;write VM_FILE
;;ZE_WRITE_VM_FILE,rrev,vrev,sigmaoutrev,denoutrev,clumpoutrev,ndrev,'/Users/jgroh/ze_models/timedep_armagh/'+modelout+'/VM_FILE_MERGE'  
;;
;;;write RVSIG_COL
;;ZE_WRITE_RVSIG_COL,rrev,vrev,sigmaoutrev,ndrev,'/Users/jgroh/ze_models/timedep_armagh/'+modelout+'/RVSIG_COL_MERGE'  
;
