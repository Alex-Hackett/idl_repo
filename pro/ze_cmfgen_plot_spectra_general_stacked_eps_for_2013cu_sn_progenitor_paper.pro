PRO ZE_CMFGEN_PLOT_SPECTRA_GENERAL_STACKED_EPS_FOR_2013CU_SN_PROGENITOR_PAPER,x,y,xtitle,ytitle,label,$
                                ...
                                pointsx2=pointsx2,pointsy2=pointsy2,id_lambda=id_lambda,id_text=id_text,id_ew=id_ew,filename=filename
...
ENDIF ELSE BEGIN 
    xr=x
    yr=y
    IF N_elements(z1) GT 1 THEN rebin_z=z1; ELSE REBIN_Z=0
ENDELSE
posminx=0.12
posmaxx=0.87
posminy=0.1
...
/nodata,POSITION=[0.12,0.13,0.97,0.97],_STRICT_EXTRA=extra
  ...
  IF ((N_elements(x2) GT 1) AND (N_elements(y2) GT 1)) THEN cgplots,x2,y2,color='red',noclip=0,linestyle=2
  IF ((N_elements(x3) GT 1) AND (N_elements(y3) GT 1)) THEN cgplots,x3,y3,color='blue',noclip=0
  IF ((N_elements(x4) GT 1) AND (N_elements(y4) GT 1)) THEN cgplots,x4,y4,color='black',noclip=0
  IF ((N_elements(x5) GT 1) AND (N_elements(y5) GT 1)) THEN cgplots,x5,y5,color='magenta',noclip=0
...
IF N_elements(id_text) GT 1 THEN BEGIN 
  id_Text_cut=id_text
