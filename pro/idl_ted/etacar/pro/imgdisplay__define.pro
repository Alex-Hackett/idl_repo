...
IF n_elements(grayimg) LT 1 THEN BEGIN
    CASE sctype OF
        ...
        grayimg = bytscl(alog10((image-xmin)>((xmax-xmin)*1e-5)), $
			min=alog10((xmax-xmin)*1e-5),max=alog10(xmax-xmin), $
			top=nc-1)+bottom
        END
        2:  BEGIN  
        IF self.dump THEN print,'SQRT SCALING'
        grayimg = bytscl(sqrt((image-xmin)>0),min=0,max=sqrt(xmax-xmin), $
			top=nc-1)+bottom
        END
        3:  BEGIN  
        IF self.dump THEN print,'HIST SCALING'
        grayimg = hist_equal(image,minv=xmin,maxv=xmax,top=nc-1)+bottom
        END
    ENDCASE
ENDIF
