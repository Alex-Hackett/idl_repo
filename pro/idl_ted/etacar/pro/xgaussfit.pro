...
;	Mar, 2004, Lindler, added another digit to the displayed coefficient 
;			fields.
;-
;-------------------------------------------------------------------------
;========================================================= XGAUSSFIT_EVENT
...
pro xgaussfit_event,event
	...
	'FIT': begin
		if base.state eq 'ADD' then begin
			...
			widget_control,base.del_field,sensitive=1
	...
	'ADD': begin
		if base.state eq '' then begin
			...
			widget_control,base.del_field,sensitive=0
		   end else begin
		        ngauss = ngauss + 1
			base.state = ''
			widget_control,base.slider,sensitive=0,set_v=1
	...
	'DELETE': begin
		isel = event.value - 1
		widget_control,base.del_field,set_value=' '
...
pro xgaussfit_setcoef,base,coef,ngauss,npoly
	...
	if ngauss gt 0 then begin
	    for i=0,ngauss-1 do begin
	    	pos = npoly + i*3
		widget_control,base.centers(i), $
				set_v=string(coef(pos+1),'$(G13.7)'),sensitive=1
		widget_control,base.peaks(i), $
				set_v=string(coef(pos),'$(G13.7)'),sensitive=1
		widget_control,base.sigmas(i), $
				set_v=string(coef(pos+2),'$(G13.7)'),sensitive=1
	    end
	end
		
	for i=0,2 do widget_control,base.poly(i),set_v = '0', sensitive = 0
	if npoly gt 0 then for i=0,npoly-1 do $
		widget_control,base.poly(i), $
				set_v = string(coef(i),'$(G13.7)'),sensitive = 1
	return
end
...
;
	...
	del_field = cw_field(menu,xsize=13,title='Delete Gaussian:',/row, $
			uvalue='DELETE',value=' ',/string,/return_events)
	cbase = widget_base(basex,/row,y_scroll_size=320,x_scroll_size=350)
	base1 = widget_base(cbase,/col)
	label = widget_label(base1,value='Centers')
	centers = lonarr(50)
	for i=0,49 do centers(i)=cw_field(base1,xsize=13, $
			title=strtrim(i+1,2),/row, $
			uvalue='COEF',value='0.0',/return_events)
	base1 = widget_base(cbase,/col)
	label = widget_label(base1,value='Sigmas')
	sigmas = lonarr(50)
	for i=0,49 do sigmas(i)=cw_field(base1,xsize=11,title=' ',/row, $
			uvalue='COEF',value='0.0',/return_events)
	base1 = widget_base(cbase,/col)
	label = widget_label(base1,value='Peaks')
	Peaks = lonarr(50)
	for i=0,49 do Peaks(i)=cw_field(base1,xsize=13,title=' ',/row, $
			uvalue='COEF',value='0.0',/return_events)
...
;
	...
	base = {plot1:plot1,xmin:xmin,xmax:xmax,ymin1:ymin1,ymin2:ymin2, $
		...
		del_field:del_field,basex1:basex1,cbase:cbase,row1:row1, $
