...
if event.id eq (*pinfo).lineplot_button then begin

	tvlct,rsave,gsave,bsave,/get
	sz = size(*self.image)
	xdim = sz[1] & ydim = sz[2]
	if rc eq 0 then begin		;row plot
		y0 = (*pinfo).y0
	    	y1 = ((y0 - long(wid/2.0))>0)<(ydim-1)
	    	y2 = ((y1 + long(wid) - 1)>y1)<(ydim-1)

		norders = n_elements(orders)
		ny_per_order = ydim/norders 
		ipos = y0/ny_per_order
		y1 = y1 mod ny_per_order - 20
		y2 = y2 mod ny_per_order - 20	
		ptitle = strtrim(sxpar(*self.header,'entry'),2)+'  Order '+ $
			strtrim(orders(ipos),2)+'  '+strtrim(y1,2)+':'+ $
			strtrim(y2,2)
		lineplot,warray(*,ipos),tot,title=ptitle
	   end else begin		;column plot
	   	ptitle = strtrim(sxpar(*self.header,'entry'),2)+'  '+title
		lineplot,findgen(n_elements(tot)),tot,title=ptitle
	end
end
...
lineplot_button = widget_button(row1,value='LINEPLOT',/no_rel)
...
popup_info = ptr_new({tlb:tlb, plot_id:plot_id, $
                ...
                decr_button:decr_button, lineplot_button:lineplot_button})
