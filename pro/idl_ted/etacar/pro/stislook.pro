;+
;		STIS quicklook analysis widget
; NAME:
;	stislook.pro
;
; PURPOSE:
...
;
; SUPPORT PROCEDURES:
;	stislook_event - main event handler for stislook.
;	pdmenu11_event - event handler for pull-down menus.
;	stl_check_file - checks for existance of a file.
;	stl_displ_win - event handler for the image display window.
;	stl_displ_image - displays an image.
;	stl_scale_im - scales an image.
;	stl_load_buffer - loads an image into a pixel map buffer.
;	stl_replot_zoom - loads/refreshes an image section in the zoom window.
;	stl_get_rtfile_name - strips off the extention from a filename.
;	stl_subtract - subtracts one image from another.
;	stl_addmess - add a message to the message window.
;
; OTHER ROUTINES UTILIZED:
;       	             	gettok.pro              stis_acquire_eds.pro
;	avg.pro 		hlist.pro	 	stis_acquire_sci.pro
;	big_ps.pro		host_to_ieee.pro 	stis_acquire_setup.pro
;	break_path.pro  	htool_add.pro	 	stis_acquire_udl.pro
;	column_sum.pro  	ieee_to_host.pro 	stis_time.pro
;	conv_vax_unix.pro	imstat.pro	 	stislook.pro
;	cross_section.pro	imsubtract.pro   	stl_blink.pro
;	curfit.pro		irafrd.pro	 	stl_mkwin.pro
;	datatype.pro		isarray.pro	 	stl_overscan.pro
;	fdecomp.pro	 	message_popup.pro	stl_rpower.pro
;	fields_popup.pro 	minmax.pro		sxaddhist.pro
;	find_with_def.pro	mkhdr.pro		sxaddpar.pro
;	fits_close.pro   	os_family.pro		sxdelpar.pro
;	fits_open.pro	 	pickfile.pro ***	sxhmake.pro
;	fits_read.pro	 	plot_iio.pro		sxhread.pro
;	fits_write.pro   	plot_popup.pro  	sxopen.pro
;	fixpix.pro	 	polyfit_popup.pro	sxpar.pro
;	flatten.pro	 	q_popup.pro		sxread.pro
;	funct_gauss.pro  	read_mama.pro		tabinv.pro
;	get_date.pro	 	readimage.pro		tag_exist.pro
;	getlog.pro	 	row_sum.pro		wherenan.pro
;	stis_read.pro      	stis_acquire.pro        xbits.pro
;
;	*** special modified version
;
; EXAMPLES:
;	1) To run program with an IRAF, FITS, SDAS, or MAMA image:
;		IDL>stislook		;see PROCEDURE above
;	2) To load 1 image and header from IDL variables:
;		IDL>stislook, image, header1=h1
;	3) To load 2 images & headers from IDL variables:
;		IDL>stislook, image1, header1=h1, image2, header2=h2
;	4) To load an image and its bias frame from IDL variables:
;		IDL>stislook, image, header1=h, bias=bias
;	
...
;	 Convert bytarr to integer to fix string conversion. TLB 20 Nov 98
;	 Force Zoom scale slider to be 256 pixels for IDL V5.2 TLB 8 Dec 98
;	 Doesn't crash if mouse button is pressed, then cursor is moved
;	   outside of display window.  TLB 18 Dec 98
;	 Now handles small images in zoom window (>256X256).  TLB 19 May 99
;	 Now handles byte arrays.  TLB  19 May 99
;	 Now Checks for multiple readouts in FITS files.  TLB  12 Aug 99
;	 Changed FITS reader from stis_read to fits_read for generic
;	   FITS files only.   TLB  28 Oct 99
;	 Fixed zoom flag reset problem.  TLB  3 Feb 2000
;	 Fixed image scaling problem with tif & gif file output.  TLB 11 Aug 00
;	 Modified for IDL 5.4 compatibilty.  TLB  21 Mar 2001
;	 Fixed window port resizing.  TLB 27 Dec 2001
;	 Window resizing - reworked.  TLB 28 Jan 02
;-
...
ds = fl_pars.dwin_size
  ...
  '  Image Processing  .Convert to Count Rate': BEGIN	;count rate conversion
	...
	if (exp eq 0) then begin
	    ...
	    stl_addmess, mess_array, message, wid_pars.mess
	    print, string(7b)
	    wait, 2.0
	endif else begin
	    ...
	    stl_load_buffer, resized_im, im_pars.pixmaps(mp), $
		wid_pars.im_win, ds, fl_pars.displ_scale(mp), fl_pars.scalev(mp,*)
	    if (fl_pars.zoom(mp)) then stl_replot_zoom, image, im_pars, $
		fl_pars, wid_pars
	    hist = 'Converted to count rate: ' + systime(0)
	    sxaddhist, hist, header			;update the header
	endelse
	stl_addmess, mess_array, 'Ready.', wid_pars.mess
    END
  ...
  '  Image Processing  .Fix Bad Column(s)': BEGIN	;bad pixel replacement
	...
	if (output.cancel eq 0) then begin		;valid ranges?
		...
		resized_im = frebin(image, ds(0), ds(1))
	        stl_load_buffer, resized_im, im_pars.pixmaps(mp), $
		    wid_pars.im_win, ds, fl_pars.displ_scale(mp), fl_pars.scalev(mp,*)
	    	if (fl_pars.zoom(mp)) then stl_replot_zoom, image, $
  ...
  '  Image Processing  .Bias Subtraction': BEGIN	;bias subtraction
	...
	stl_subtract, image, resized_im, header, 'bias', im_pars, $
	    fl_pars, wid_pars, mess_array
    END

  '  Image Processing  .Dark Subtraction': BEGIN	;dark subtraction

	stl_subtract, image, resized_im, header, 'dark', im_pars, $
	    fl_pars, wid_pars, mess_array
    END

  '  Image Processing  .Flat Field': BEGIN		;flat field

	stl_addmess, mess_array, 'Flat fielding...', wid_pars.mess
	...
	if (err eq 1) then begin			;operation unsucessful
	    print, string(7b)
	    stl_addmess, mess_array, err_mess, wid_pars.mess
	    wait, 2.0
	endif else begin				;it worked!
	    mess = 'Operation successful - displaying processed image...'
	    stl_addmess, mess_array, mess, wid_pars.mess
	    ds = fl_pars.dwin_size
	    resized_im = frebin(image, ds(0), ds(1))
	    stl_load_buffer, resized_im, im_pars.pixmaps(mp), $
		wid_pars.im_win, ds, fl_pars.displ_scale(mp), fl_pars.scalev(mp,*)
  ...
  '  Image Processing  .Rebin: Hi to Lo-res': BEGIN	;Rebin
  ...
  '  Image Filtering  .Convolution': BEGIN		;convolution
	...
	if (infile ne '') then begin
	    widget_control, /hourglass
	    stl_addmess, mess_array, 'Convolving...', wid_pars.mess
  ...
  '  Rotate  .Transpose Y -> -Y': BEGIN
   ...
   '  Image Info  .Statistics': BEGIN
	...
	off = [600,0]
	stl_addmess, mess_array, mess_arr, wid_pars.mess
	stl_addmess, mess_array, 'Ready.', wid_pars.mess
...
if (release ne 3.6) then begin			;retrieve structures
    ...
    handle_value, ptrs.im_ptr, im_pars, /no_copy
    handle_value, ptrs.fl_ptr, fl_pars, /no_copy
    handle_value, ptrs.wid_ptr, wid_pars, /no_copy
    handle_value, ptrs.im1_ptr, image, /no_copy
    handle_value, ptrs.im1r_ptr, resized_im, /no_copy
    handle_value, ptrs.im2_ptr, image2, /no_copy
    handle_value, ptrs.im2r_ptr, resized_im2, /no_copy
    handle_value, ptrs.hd_ptr, header, /no_copy
    handle_value, ptrs.hd2_ptr, header2, /no_copy
    handle_value, ptrs.ms_ptr, mess_array, /no_copy
endelse
  ...
  '1': BEGIN						;FILE menu
	...
	CASE 1 OF
		...
		if (outfile ne '' and goodfile eq 1) then begin
		    ...
		    stl_scale_im, tmp_im, type, im, scalev
	    ...
	    if (fl_pars.file_type eq 'ENTRY') then begin	;DB entry			
	        ...
	        label = ['Entry:']				
	        sw_l = ['PRELAUNCH','POSTLAUNCH']
	        sw= ' '
	        sw_def = 2
	        edat = fields_popup('STIS_READ', q, label, bswitch=sw, $
	            sw_label_arr=sw_l, def_sw=2, grl=event.top)
	        if (edat.cancel eq 0) then begin	
	            ...
	            if (abs_entry gt n_entries(0)) then begin	;Check for
		        infile = ''				;valid entry #
		        newpath = ''
		    	mess = 'Entry number out of range!'
		...
		stl_addmess, mess_array, message, wid_pars.mess
	    	...
	    	if (type eq '/ENTRY') then begin
	    	    if (edat.sw_num eq 1) then begin
	    	        stis_read, entry, h, im, /no_abort, /pre
	    	    endif else begin
	    	    	nr = dbval(entry,'NREADS')
	    	    	if (nr gt 1) then begin
	    	    	    stnr = strcompress(string(nr), /remove_all)
	    	    	    wtt = 'Multiple Readouts'
	    	    	    q1 = 'Image has ' + stnr + ' readouts.'
	    	    	    q = q1 + ' Enter desired readout #'
	    	    	    label = ['Readout:']
	    	    	    rdat = fields_popup(wtt,q,label,grl=event.top)
	    	    	    rdout = rdat.values(0)
	    	    	    strdout = strcompress(string(rdout), /remove_all)
	    	    	    widget_control, /hourglass
	    	    	    if (rdat.cancel eq 0) then begin
	    	    	        mess = 'Loading readout # ' + strdout
	    	    	        stl_addmess, mess_array, mess, wid_pars.mess
	    	                stis_read, entry, h, im, /no_abort, $
	    	        	    /post, readout=rdout
	    	        	infile = infile + ' Readout ' + strdout
	    	       	    endif else begin
	    	       	        mess = 'Image load aborted.'
	    	       	        stl_addmess, mess_array, mess, wid_pars.mess
	    	       	    	goto, skip_alot
	    	       	    endelse
	    	        endif else begin
	    	            stis_read, entry, h, im, /no_abort, /post
	    	        endelse
	    	    endelse
	    	    if !err lt 0 then begin
	    	        message = 'Error opening file! Operation aborted.'
	    	        print, string(7b)
	    	        junk = widget_message(message, /error, $
		...
		if (type eq '/FITS') then begin
		    ...
		    fits_read,fcb,d,h1,/header_only
		    images = where(fcb.extname eq 'SCI',nr)
		    fits_close,fcb
		    if (nr gt 1) then begin
		    	stnr = strcompress(string(nr), /remove_all)
	    	    	wtt = 'Multiple Readouts'
	    	    	...
	    	    	if (rdat.cancel eq 0) then begin
	    	    	    mess = 'Loading readout # ' + strdout
	    	    	    stl_addmess, mess_array, mess, wid_pars.mess
	    	            stis_read, infile, h, im, /no_abort, $
	    	        	/post, readout=rdout
	    	            infile = infile + ' Readout ' + strdout
	    	       	endif else begin
	    	       	    mess = 'Image load aborted.'
		    	...
		    	fits_read, infile, im, h, /no_abort
		    endelse
		    if !err lt 0 then begin
		    	message = 'Error opening file! Operation aborted.'
		    	print, string(7b)
	    	    ...
	    	    com = 'readimage, infile, im, h, ' + type
	    	    r = execute(com)
	    	endelse

; check for time-tag data

		obsmode = strtrim(sxpar(h, 'OBSMODE'))
		if (!err ge 0) then begin
		    if (obsmode eq 'TIME-TAG') then begin
		        mess = 'Note: Displaying reconstructed time-tag image.'
		        stl_addmess, mess_array, mess, wid_pars.mess
		    endif
		endif
...
; check for MIE (memory dump) image
		...
		im = im*1
		    ...
		    if (mp eq 0) then begin		;buffer 1 selected
			    ...
			    stl_load_buffer, resized_im2, im_pars.pixmaps(mp), $
	    		    	wid_pars.im_win, ds, fl_pars.displ_scale(mp), scalev
			    s = size(image2)
			    if (fl_pars.num_images eq 2) then begin
			    	wset, im_pars.pixmaps(mp+2)
	    	...
	    	if (naxis eq 2) then begin		;update database if
	    	    ...
	    	    stl_get_rtfile_name, infile, newpath, $
...
skip_alot:
	...
	if (saving eq 1) then begin			;Saving file
	    ...
	    outfile = pickfile(path=fl_pars.path, file=def_file, $
	    	title=' Name the output FITS file ')

	    if (outfile ne '') then begin
	    	s = strpos(outfile, fextent)
	    	if (s eq -1) then outfile = outfile + fextent
	    	stl_check_file, outfile, answer, event.top
		    ...
		    scalev = fl_pars.scalev(mp,*)
		    sc_minmax = scalev
		    stl_scale_im, im, type, scaled_im, sc_minmax
		    ...
		    case ftype of
		    	...
		    	'TIFF': tiff_write, outfile, $
			    bytscl(scaled_im,min=minv,max=maxv,top=topv)
		    	'JPEG': write_jpeg, outfile, $
			    bytscl(scaled_im,min=minv,max=maxv,top=topv)
		    	'GIF': write_gif, outfile, $
			    bytscl(scaled_im,min=minv,max=maxv,top=topv)
		    endcase
  ...
  '4': BEGIN						;zoom window events
	...
	if (x ge 0 and x le mx and y ge 0 and y le my) then begin
	    if (mp eq 0) then z = float(image(x,y))
	    if (mp eq 1) then z = float(image2(x,y))
  ...
  '5': BEGIN						;change zoom scale
	...
	fl_pars.zoom_scale(mp) = zm
	widget_control, /hourglass
	if (mp eq 0 and fl_pars.zoom(mp)) then $
	    stl_replot_zoom, image, im_pars, fl_pars, wid_pars
	if (mp eq 1 and fl_pars.zoom(mp)) then $
	    stl_replot_zoom, image2, im_pars, fl_pars, wid_pars
      	END
  ...
  '6': BEGIN						;change display scale
	...
	if (mp eq 0) then begin				;reload buffers
	    stl_load_buffer, resized_im, im_pars.pixmaps(mp), wid_pars.im_win, $
		fl_pars.dwin_size, fl_pars.displ_scale(mp), scalev
	    if (fl_pars.zoom(mp)) then stl_replot_zoom, image, im_pars, $
		fl_pars, wid_pars
	endif else begin
	    stl_load_buffer, resized_im2, im_pars.pixmaps(mp), wid_pars.im_win, $
		fl_pars.dwin_size, fl_pars.displ_scale(mp), scalev
	    if (fl_pars.zoom(mp)) then stl_replot_zoom, image2, im_pars, $
		fl_pars, wid_pars
	endelse
  ...
  '8': BEGIN						;blink control panel
	stl_blink, im_pars, fl_pars, wid_pars, group=event.top
	END
  ...
  '9a': BEGIN						; Print Header
  	...
  	mess = 'Printing header on printer ' + printer
  	stl_addmess, mess_array, mess, wid_pars.mess
	if (fl_pars.mapped eq 0) then begin
	...
	endelse
	spawn, 'lpr -P' +  printer + ' /tmp/header.tmp'
  ...
  '12': begin						;help!
	help_file = find_with_def('stislook.hlp', 'SDOC', /nocur)
	xdisplayfile, help_file, group=event.top, title='Help'
	end

ENDCASE

if (release ne 3.6) then begin				;put away structures
...
endif else begin					;v3.6 handles
    handle_value, ptrs.im_ptr, im_pars, /no_copy, /set	
    handle_value, ptrs.fl_ptr, fl_pars, /no_copy, /set	
    handle_value, ptrs.wid_ptr, wid_pars, /no_copy, /set
    handle_value, ptrs.im1_ptr, image, /no_copy, /set
    handle_value, ptrs.im1r_ptr, resized_im, /no_copy, /set
    handle_value, ptrs.im2_ptr, image2, /no_copy, /set
    handle_value, ptrs.im2r_ptr, resized_im2, /no_copy, /set
    handle_value, ptrs.hd_ptr, header, /no_copy, /set
    handle_value, ptrs.hd2_ptr, header2, /no_copy, /set
    handle_value, ptrs.ms_ptr, mess_array, /no_copy, /set
endelse
...
pro stl_check_file, file, answer, grl			
...
if (event.id eq event.top) then begin			;resize the display
    widget_control, /hourglass
    stl_addmess, mess_array, 'Resizing...', wid_pars.mess
    newxs = event.x - 6
    newys = event.y - 36
    fl_pars.dwin_size = [newxs,newys]
    widget_control, wid_pars.displ_base, tlb_get_offset=off
    widget_control, wid_pars.displ_base, /destroy
    stl_mkwin, 'stl_displ_win', draw_id, win_id, c_labels, xsize=newxs, $
	ysize=newys, xoff=off(0), yoff=off(1), gleader=wid_pars.gleader
    ...
    for n = 0,1 do begin				;re-make pixel maps
    	...
    	window, /FREE, /PIX, xs=newxs, ys=newys
    	im_pars.pixmaps(n) = !d.window
    endfor
    
    if (num_images ne 0) then begin		; no images loaded
    	resized_im = frebin(image, newxs, newys)
    	if (fl_pars.num_images eq 2) then $
    	    resized_im2 = frebin(image2, newxs, newys)
		...
		wset, im_pars.pixmaps(1)
    		stl_displ_image, resized_im2, fl_pars.displ_scale(1), $
		    fl_pars.scalev(mp,*)
	    endif
    	endif else begin
	    stl_load_buffer, resized_im2, im_pars.pixmaps(mp), wid_pars.im_win, $
	    ...
	    wset, im_pars.pixmaps(0)
    	    stl_displ_image, resized_im, fl_pars.displ_scale(0), $
		fl_pars.scalev(mp,*)
    	endelse
    	stl_addmess, mess_array, 'Ready', wid_pars.mess
    endif
	...
	case fl_pars.plot_type of
		...
		widget_control, wid_pars.zoom_draw, sensitive=1
	    	...
	    	im_pars.cenx = cenx
	    	im_pars.ceny = ceny
	       	stl_replot_zoom, working_image, im_pars, $
		    fl_pars, wid_pars
	   	widget_control, wid_pars.zoom_draw, sensitive=1
		end
	 endcase
	...
	if ( cenx gt 0 and cenx lt im_pars.nx(mp) and $
	    ceny gt 0 and ceny lt im_pars.ny(mp) ) then begin
...
pro stl_displ_image, image, type, scalev

; Displays image according to scale type. 

stl_scale_im, image, type, scaled_im, scalev
...
pro stl_scale_im, image, type, scaled_im, sc_minmax
...
pro stl_load_buffer, image, pixmap, im_win, win_size, scale, scalev

; loads image into the pixel map buffer and copies it to the display window.
 
wset, pixmap

stl_displ_image, image, scale, scalev
...
pro stl_replot_zoom, image, im_pars, fl_pars, wid_pars	;displays the zoomed
							;image
...
x2_tmp = fix(x1 + xs/zm)                                ;to zoom in on
y1 = fix(ceny - ys/(zm*2.0))
y2_tmp = fix(y1 + ys/zm)
sub_xs = x2_tmp - x1
sub_ys = y2_tmp - y1

if (x2_tmp gt nx-1) then begin                          ;make sure section does
    ox2 = x2_tmp - nx + 1                               ;not run off top or the
    x2 = nx - 1                                         ;right side of image
endif else begin
    ox2 = 0
    x2 = x2_tmp
endelse

if (y2_tmp gt ny-1) then begin
    oy2 = y2_tmp - ny + 1
    y2 = ny - 1
endif else begin
    oy2 = 0
...
subim = fltarr(sub_xs+1, sub_ys+1)
...
subim(sx1:sx2,sy1:sy2) = image(ix1:x2,iy1:y2)

im_pars.sx1(mp) = sx1                                   ;save coord. systems
im_pars.sy1(mp) = sy1
im_pars.ix1(mp) = ix1
im_pars.iy1(mp) = iy1
...
stl_load_buffer, subim, im_pars.pixmaps(mp+2), $
...
pro stl_get_rtfile_name, infile, path, type, rtfile
...
pro stl_subtract, image, resized_im, header, type, im_pars, fl_pars, $
   wid_pars, mess_array
...
pro stl_addmess, mess_array, message, mess_wid
...
PRO stislook, image, image2, header1=header, header2=header2, bias=bias, $
   dark=dark, flat=flat, log=log, sqt=sqt, hist=hist, ixsize=ixsize, $
   iysize = iysize

if n_elements(ixsize) eq 0 then ixsize = 1024
if n_elements(iysize) eq 0 then iysize = 512
...
case n_params() of 
    ...
    1: begin
        image = image*1					;in case of bytarr
	num_images = 1
	im = image & im2 = 0
	s = size(image)
	nx = [s(1), 0] & ny = [s(2), 0]
	...
	header2 = strarr(20)
	resized_im = frebin(im, ixsize, iysize)
	resized_im2 = 0
	lfile_type = ['IDL', '']
        scalev(0,*) = minmax(im)
	end
	...
	resized_im = frebin(im, ixsize, iysize)
	resized_im2 = frebin(im2,ixsize , iysize)
	lfile_type = ['IDL', 'IDL']
        scalev(0,*) = minmax(im)
        scalev(1,*) = minmax(im2)
	end
...
stislook = WIDGET_BASE( COLUMN=1, TITLE='STISLook', xoffset=0, yoffset=0)
  ...
  BASE6 = widget_base(stislook, /row)
    MenuDesc524 = [ $
	...
	{ CW_PDMENU_S,       0, 'Fix Bad Column(s)' }, $ ;        3
        ...
        { CW_PDMENU_S,       2, 'Rebin: Hi to Lo-res' }, $ ; 7
  ...
  BASE16 = WIDGET_BASE(stislook, ROW=1)
...
WIDGET_CONTROL, stislook, /REALIZE

version = widget_info(/version)
if (version.style eq 'Motif') then begin
    widget_control, stislook, tlb_set_xoffset=0
    widget_control, stislook, tlb_set_yoffset=0
endif
...
stl_mkwin, 'stl_displ_win', dbase_id, win_id, c_labels, xsize=ixsize, $
    ysize=iysize, xoff=420, yoff=350, gleader=stislook
    
pixmaps = intarr(4)
;
; big window pixmaps
;
window, /FREE, /PIX, xs=ixsize, ys=iysize
pixmaps(0) = !d.window
window, /FREE, /PIX, xs=ixsize, ys=iysize
pixmaps(1) = !d.window
;
; zoom window pixmaps for image1 and image2
;
for n = 2, 3 do begin
    window, /FREE, /PIX, xs=256, ys=256
    pixmaps(n) = !d.window
endfor
...
case num_images of
	...
	widget_control, /hourglass
	wset, pixmaps(1)
	stl_displ_image, resized_im2, scale, scalev(1,*)
	end
endcase

if (num_images ne 0) then begin				;display first frame
    wset, pixmaps(0)
    stl_displ_image, resized_im, scale, scalev(0,*)
    wset, win_id
    device, copy=[0,0,ixsize,iysize,0,0,pixmaps(0)]
...
fl_pars = {file:['IDL', 'IDL'], path:path, zoom_scale:[2,2], zoom:[0,0], $
    y_units:['counts', 'counts'], displ_scale:[scale,scale], $
    pt1:[0.0,0.0], num_images:num_images, plot_type:'', mapped:0, $
    dwin_size:[ixsize,iysize], file_type:'', filter:'', no_bias:no_bias, $
...
wid_pars = {im_win:win_id, zm_win:zoom_win, displ_base:dbase_id, $
    ...
    subbase:BASE36, subbase1:BASE43, pdmenu_base:BASE6, gleader:stislook, $
...
if (release ne '3.6') then begin		;create pointers(v3.5)
    im_ptr = widget_base()
    ...
    im_ptr = handle_create(value=im_pars, /no_copy)	;create handles(v3.6.x)
    fl_ptr = handle_create(im_ptr, value=fl_pars, /no_copy)
    wid_ptr = handle_create(im_ptr, value=wid_pars, /no_copy)
    im1_ptr = handle_create(im_ptr, value=im, /no_copy)
    im1r_ptr = handle_create(im_ptr, value=resized_im, /no_copy)
    im2_ptr = handle_create(im_ptr, value=im2, /no_copy)
    im2r_ptr = handle_create(im_ptr, value=resized_im2, /no_copy)
    hd_ptr = handle_create(im_ptr, value=header)
    hd2_ptr = handle_create(im_ptr, value=header2)
    ms_ptr = handle_create(im_ptr, value=mess_array)
endelse
...
widget_control, stislook, set_uvalue=pointers		;stick the pointers
							;into the user value
							;of the base widget  
