...
PRO ZE_COMPUTE_IMAGE_DENCON_X_Y_V2,pa,rstar,dist,lambda_val,lambda0,lambda,np,ndelta1,nos,ipdelta,p,delta1_vec,x,y,den,circ_GridSpace_mas,intens2d_lambda,$
...
;circ_GriddedData=ROT([i1[0:pix/2 -1,*],i2[1:pix/2,*]],0,/INTERP)
;circ_GriddedData=ROT(circ_GriddedData,0,/INTERP)
END
;--------------------------------------------------------------------------------------------------------------------------;----------------------------------------------------------------------
...
$MAIN CODE
;
;PRO ZE_READ_2D_RV_DATA_V6
;V5:  plot the full star (2pi) instead of only half star (pi)
;V6:  input data has to go from 0 to 2PI instead of 0 to PI i.e. does not work for cavity models! If you are interested in cavity models, use V5
close,/all
...
dir='/Users/jgroh/ze_models/2D_models/etacar/mod31_groh/'
...
;sufix='98_41' ;f1g
sufix='90_45_vrot'
;model='tilted_owocki_obl'
;sufix='70_85';f2a
;sufix='50_40'
;model='tilted_owocki'
;sufix='95_60_halpha';f2a
...
readf,lun,FORMAT='(1x,I,10x,A23)',ND,desc1   ;works as long as ND < 100
readf,lun,FORMAT='(I,10x,A22)',beta,desc2 ;works as long as beta < 100
N=ND*beta
...
rvtj='/Users/jgroh/ze_models/etacar/mod31_groh/RVTJ'
...
;;for density
;ZE_COMPUTE_IMAGE_DENCON_X_Y_V2,pa,rstar,dist,lambda_val,lambda0,lambda,ND,beta,nos,dencon,r1,betaang,x,y,den,circ_GridSpace_mas,intens2d_lambda,$
;circxvector,circyvector,circ_griddedData
;
;omega_Str=strmid(sufix,0,2)
;;convert to w
;w_str=number_formatter(SQRT(float(omega_str)/100.0),decimals=2)
;;title=TEXTOIDL('v_{rot}/v_{crit}=')+w_str
...
;for radial velocity
den[*]=1
ZE_COMPUTE_IMAGE_DENCON_X_Y_V2,pa,rstar,dist,lambda_val,lambda0,lambda,ND,beta,nos,azivel,r1,betaang,x,y,den,circ_GridSpace_mas,intens2d_lambdanh0,$
circxvector2,circyvector2,circ_griddedData2
...
;den[*]=1
;ZE_COMPUTE_IMAGE_DENCON_X_Y_V2,pa,rstar,dist,lambda_val,lambda0,lambda,ND,beta,nos,dencon,r1,betaang,x,y,den,circ_GridSpace_mas,intens2d_lambdanh0,$
;circxvector2,circyvector2,circ_griddedData2
;
;a=min(circ_griddedData2,/NAN)
;b=max(circ_griddedData2,/NAN)
;ZE_ETACAR_CRIRES_CREATE_GENERIC_IMAGE_FROM_DATAA2,circ_griddedData2,a,b,img2,SQRT=0,LOG=0
;imgname='etc_cmfgen_plus_cavity_dencon_'+model+'_'+sufix
;
;ZE_PLOT_IMAGE_DENCON_X_Y_XWINDOW,2,img2,a,b,rstar,dist,pa,inc_str,lambda_val,lambda0,lambda,np,ndelta1,nos,ipdelta,p,delta1_vec,x,y,intens2d_lambdanh0,$
;circxvector2,circyvector2
;ZE_PLOT_IMAGE_DENCON_X_Y_EPS,img2,a,b,imgname,rstar,dist,pa,inc_str,lambda_val,lambda0,lambda,np,ndelta1,nos,ipdelta,p,delta1_vec,x,y,intens2d_lambdanh0,$
...
rot_den=dblarr(nd,beta)
FOR I=0, beta-1 DO rot_den[*,i]=denconr
FOR I=0, nd-1 DO rot_den[i,*]=denconbeta

T1=0.0D
FOR I=0, beta/2-1 DO BEGIN
T1=T1+0.25D0*(ROT_DEN(1,I)+ROT_DEN(1,I+1))*(COS(betaang(I))-COS(betaang(I+1)))
print,t1
ENDFOR
print,'Beta/2', betaang[beta/2]
