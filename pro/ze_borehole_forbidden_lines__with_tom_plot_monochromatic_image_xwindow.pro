PRO ZE_BOREHOLE_FORBIDDEN_LINES__WITH_TOM_PLOT_MONOCHROMATIC_IMAGE_XWINDOW,img,a,b,xvalues,yvalues,aa,bb,vel_val,ct,nointerp
...
plot, [min(xvalues),max(xvalues)], [min(yvalues),max(yvalues)],XTICKFORMAT='(F5.1)',xstyle=1,ystyle=1, xtitle='Spatial offset along slit (arcsec)', ytitle='Spatial offset perpendicular to slit (arcsec)',$
/NODATA, Position=[0.08, 0.18, 0.945, 0.86],XRANGE=[min(xvalues),max(xvalues)], YRANGE=[min(yvalues),max(yvalues)]
!P.Background = fsc_color('white')
!P.Color = fsc_color('black')
LOADCT, ct
;LOADCT,0
