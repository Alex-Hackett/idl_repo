...
;  ASTPATH - path to astrometry data, '/frakir/raid/buie/astrometry' 
;  REDPATH - path to reduced data, default is '/frakir/raid/reduced'
;  RAWPATH - path to image data, default is a vector:
;             ['/frakir/data1/buie/rawfits',
;              '/gw/data7/buie/rawfits', '/gw/data3/buie/rawfits']
;  KBOPATH - location of bernstein files,
;             default is '/frakir/raid/buie/kbo/Bernstein'
;  OUTPATH - location for output file, current default is 
;             '/frakir/raid/reduced/Summary/Current/'
; OUTPUTS: 
...
pro des_sum, rootid,ASTPATH=astpath, REDPATH=redpath, RAWPATH=rawpath, $
   ...
   if badpar(astpath,[0,7],0,caller='des_sum:(ASTPATH) ', $
            default='/frakir/raid/buie/astrometry') then return

   if badpar(redpath,[0,7],0,caller='des_sum:(REDPATH) ', $
            default='/frakir/raid/reduced') then return

   if badpar(rawpath,[0,7],0,caller='des_sum:(RAWPATH) ', $
            default=['/frakir/data1/buie/rawfits', $
                     '/gw/data7/buie/rawfits', $
                     '/gw/data3/buie/rawfits' ]) then return

   if badpar(kbopath,[0,7],0,caller='des_sum:(KBOPATH) ', $
            default='/frakir/raid/buie/kbo') then return
