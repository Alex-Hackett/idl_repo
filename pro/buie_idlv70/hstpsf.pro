...
;               Current default is /flora/data1/plc/hst/hstpsf. An empty
...
pro hstpsf, x, y, date, filter, bmvnum, psf, xmax, ymax,$
   ...
   if badpar(hstpath, [0,7], 0, caller=self+'(hstpath) ',$
             default='/frakir/raid/buie/hstpsf/') then return
;             default='/flora/data1//plc/hst/hstpsf/') then return
   ...
   if camera eq 2 then begin
      ...
      print,self,'Camera Number must be 2, 6  or 16.'
      return
   endelse
   ...
   endif else if camera eq 16 or camera eq 6 then begin
      path = mainpath + instdir + cameradir + bmvdir + filterdir + z4dir

      psfrootname = path + sx + '_' + sy + '_'
      if camera eq 6 then psfname = psfrootname + '00.fits' $
...
;   endif
   ...
   ;---------------------------------------------------------------------------
   if camera eq 16 or camera eq 6 or camera eq 2 then begin
      ...
      if sampfact eq 1 then $
         undersample = [ 'N' ] $
      else undersample = [ 'Y',  string(sampfact) ]

      tiny1input = string(camera)

      if camera eq 2 then tiny1input = [ tiny1input, string(chip)]; wfpc chip
      tiny1input=[tiny1input,  sx+' '+sy ]
      year = '19'+strmid(date,0,2)
      month = strmid(date,2,2)
      day = strmid(date,4,2)
      ; wfpc needs date.
      if camera eq 2 then tiny1input = $  
                          [ tiny1input, day + ' ' + month + ' ' + year]
      tiny1input=[tiny1input,  uc_filter, spectrum_choice, $
                  spectrum_item,string(psfsize) ]
      if camera ne 16 then tiny1input = [tiny1input, undersample]; sample
