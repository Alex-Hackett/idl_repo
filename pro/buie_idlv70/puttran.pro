...
pro puttran,inst,date,filter,color1,color2,tran,transig,jdref,nobs,replaced, $
   ...
   for i = 0,4 do begin
      thevalues = thevalues+commaspc+ $
                  quote(string(tran[i],FORMAT='(F9.5)'))
   endfor

   ; extinction and zero pt error term fields
   for i = 0,4 do begin
      thevalues = thevalues+commaspc+quote(string(transig[i],FORMAT='(F9.5)'))
   endfor


   ; jd for time dependent extinction term
   thevalues = thevalues+commaspc+quote(string(jdref,FORMAT='(d15.6)'))

   ; nobs

   thevalues = thevalues+fittedst
   thevalues = thevalues+commaspc+string(nobs)
