...
;     astrom,'080501',1,path='/spikard/data1/buie/rawfits'
;  In this case it would look for the file:
;     /spikard/data1/buie/rawfits/080501/080501.001   (root is added to path)
;  However,  you could do the same thing with:
;     astrom,'/spikard/data1/buie/rawfits/080501/080501',1
;  or
;     astrom,'/spikard/data1/buie/rawfits/080501/080501.001'
...
;               specified by XTITERMS and ETATERMS.  Example: [0.3,0.65] would
...
pro astrom_twostar,dv,info,sra,sdec,sx,sy,ssz,valid
...
retry:
   ...
   astrd2xy,sra,sdec,info,sx,sy,XI=sxi,ETA=seta
...
pro astrom_objectzoom,dv,info,xm,ym,fwhm0
...
;   for this to work.
            if not twostar then begin
               ...
               if doshells and ( maxrcrit lt 0 or (maxrcrit gt 0 and rcrit lt maxrcrit)) then begin
                  ...
                  r2 = sqrt((sx-info.xc0)^2 + (sy-info.yc0)^2)
;                  zr2 = where(r2 le max(r[zr[nr-1]])*1.01,count)
                  zr2 = where(r2 le max(r[zr[nr-1]])*1.01 and $
                              sx ge -info.xc*0.10  and $
                              sx le  info.xc*2.10  and $
                              sy ge -info.yc*0.10  and $
                              sy le  info.yc*2.10, count)
               ...
               angoff=0.
               if xyerror eq 1 then begin
                  frmdxyr,sx[zr2],sy[zr2],xpos[zr],ypos[zr],-1.0,1.0,.1, $
                     xoff,yoff,angoff,xyerror,nx=sz[1],ny=sz[2], $
                     tolerance=0.001,/silent
            ...
            if twostar or xyerror eq 1 then begin
               ...
               if partial_solution then begin
                  ...
                  astrom_twostar,dv,info,sra,sdec,sx,sy,ssz,valid
               endif else begin
                  astrom_twostar,dv,info,sra,sdec,sx,sy,ssz,valid
               endelse
               ...
               if headercoord then begin
                  raoff=info.racen-hra
                  if raoff gt      !pi then raoff=raoff-2.0d0*!dpi
                  if raoff lt -1.0*!pi then raoff=raoff+2.0d0*!dpi
                  raoff=raoff*cos(info.deccen)
                  decoff=info.deccen-hdec
                  print,'new raoff,decoff ',raoff,decoff
                  print,'new scale,angle ',info.pscale,info.rang
               endif
            endif else begin
               angle_r = angoff/!radeg
               sx =  (sx-sz[1]/2)*cos(angle_r)+(sy-sz[2]/2)*sin(angle_r) + $
                         sz[1]/2 + xoff
               sy = -(sx-sz[1]/2)*sin(angle_r)+(sy-sz[2]/2)*cos(angle_r) +  $
                         sz[2]/2 + yoff
            endelse
...
restart_shell:
                  ...
                  if worstresid gt residcap then begin
                     if batch or ishell ne 0 then begin
                        ...
                        ;astrom_twostar,dv,info,sra,sdec,sx[zr2],sy[zr2],ssz,valid
                        astrom_twostar,dv,info,sra,sdec,sx,sy,ssz,valid
                        if not valid then begin
                           logerror,errfile,'Manual solution aborted, file '+imfile+exttag
                           countgood=0
                  ...
                  sx = (sx-info.xc0)/renormfac
                  sy = (sy-info.yc0)/renormfac
                  astinvrt,xi,cxi,txiterms,eta,ceta,tetaterms,sx,sy
                  sx = sx*renormfac + info.xc0
                  sy = sy*renormfac + info.yc0
                  if not nodisplay then begin
                     astrom_dim,window,bim,binfac,border
                     for i=0L,n_elements(sx)-1 do $
                        oplot,[sx[i]],[sy[i]],psym=4,color=color2,symsize=ssz[i]
...
;photographic refinement
                  ...
                  xpos[iz]=xm
                  ypos[iz]=ym
                  perr[iz]= sqrt((xm-sx[iz])^2 + (ym-sy[iz])^2)*info.pscale
                  fwhm[iz]=fwhm0
                  mag[iz]=mag0
                  err[iz]=err0
                  sig_max[iz]=max0
...
;                     oplot,astx,asty,psym=8,color=color1,symsize=0.2
               ...
               if !mouse.button eq 1 then begin
                  ...
                  repeat begin
                     astrom_objectzoom,dv,info,xm,ym,fwhm0
...
next_extension:
      ...
      if countgood ne 0 then begin
         z=where(xiterms eq 1,count)
         jt=0
         for it=0,count-1 do begin
            tcxi[z[it]]  = cxi[jt]
            tceta[z[it]] = ceta[jt]
            jt++
         endfor
      ...
      if extlist[ext] eq 0 then begin
         ...
         if ext eq 0 then begin
            fitdata = { $
               nstars:    lonarr(numext), $ ;
               xc:        fltarr(numext), $ ; 
               yc:        fltarr(numext), $ ; 
               cra:       dblarr(numext), $ ; 
               cdec:      dblarr(numext), $ ; 
               photzp:    fltarr(numext), $ ; 
               xi_const:  dblarr(numext), $ ; 
               xi_x:      dblarr(numext), $ ; 
               xi_y:      dblarr(numext), $ ; 
               xi_r:      dblarr(numext), $ ; 
               xi_xx:     dblarr(numext), $ ; 
               xi_yy:     dblarr(numext), $ ; 
               xi_xxx:    dblarr(numext), $ ; 
               xi_yyy:    dblarr(numext), $ ; 
               xi_xyy:    dblarr(numext), $ ; 
               xi_xxy:    dblarr(numext), $ ; 
               xi_scat:   fltarr(numext), $ ; 
               eta_const: dblarr(numext), $ ; 
               eta_x:     dblarr(numext), $ ; 
               eta_y:     dblarr(numext), $ ; 
               eta_r:     dblarr(numext), $ ; 
               eta_xx:    dblarr(numext), $ ; 
               eta_yy:    dblarr(numext), $ ; 
               eta_xxx:   dblarr(numext), $ ; 
               eta_yyy:   dblarr(numext), $ ; 
               eta_xyy:   dblarr(numext), $ ; 
               eta_xxy:   dblarr(numext), $ ; 
               eta_scat:  fltarr(numext)  $ ; 
               }
         endif
         fitdata.nstars[ext]    = countgood
         fitdata.xc[ext]        = info.xc0
         fitdata.yc[ext]        = info.yc0
         fitdata.cra[ext]       = info.ra0
         fitdata.cdec[ext]      = info.dec0
         fitdata.photzp[ext]    = magdiff
         fitdata.xi_const[ext]  = tcxi[0]
         fitdata.xi_x[ext]      = tcxi[1]
         fitdata.xi_y[ext]      = tcxi[2]
         fitdata.xi_r[ext]      = tcxi[3]
         fitdata.xi_xx[ext]     = tcxi[4]
         fitdata.xi_yy[ext]     = tcxi[5]
         fitdata.xi_xxx[ext]    = tcxi[6]
         fitdata.xi_yyy[ext]    = tcxi[7]
         fitdata.xi_xyy[ext]    = tcxi[8]
         fitdata.xi_xxy[ext]    = tcxi[9]
         fitdata.xi_scat[ext]   = xiscat
         fitdata.eta_const[ext] = tceta[0]
         fitdata.eta_x[ext]     = tceta[1]
         fitdata.eta_y[ext]     = tceta[2]
         fitdata.eta_r[ext]     = tceta[3]
         fitdata.eta_xx[ext]    = tceta[4]
         fitdata.eta_yy[ext]    = tceta[5]
         fitdata.eta_xxx[ext]   = tceta[6]
         fitdata.eta_yyy[ext]   = tceta[7]
         fitdata.eta_xyy[ext]   = tceta[8]
         fitdata.eta_xxy[ext]   = tceta[9]
         fitdata.eta_scat[ext]  = etascat
      endelse
