PRO ZE_CMFGEN_PLOT_SPECTRA_GENERAL_STACKED_EPS,x,y,xtitle,ytitle,label,$
...
ENDIF ELSE BEGIN 
     IF n_elements(labelz) LT 1 THEN labelz=''
       filename_prefix='/Users/jgroh/temp/output_evol_'
      filename_sufix='/Users/jgroh/temp/output_evol_'+xtitle+'_'+ytitle+'_'+label+'_colorcoded_'+labelz+'.eps'
      ZE_REMOVE_SPECIAL_CHARACTERS_FROM_STRING,filename_sufix
      psfilename=filename_prefix+filename_sufix
ENDELSE
...
/nodata,POSITION=[0.095,0.23,0.98,0.98],_STRICT_EXTRA=extra
  ...
  IF ((N_elements(x4) GT 1) AND (N_elements(y4) GT 1)) THEN cgplots,x4,y4+4,color='dark green',noclip=0
...
;xyouts,5100,2.5,'AG Car observations 2002-Jul-04',/DATA,color=fsc_color('dark green'),charsize=2.5
;xyouts,5100,8.0,'pre-SN 20 Msun rot. model',/DATA,color=fsc_color('black'),charsize=2.5
...
If KEYWORD_SET(id_text) THEN BEGIN
  ...
  for l=1., n_elements(id_lambda)-1. do begin
    if (id_lambda[l] gt 3910) and (id_lambda[l] lt 6500) and (abs(id_ew[l]) gt 0.3) then begin
    ...
    endif else xyouts,id_lambda[l]+15,16.3,'!3'+string(45B)+id_text_cut[l],alignment=0.5,orientation=90,charsize=2.8,charthick=12
