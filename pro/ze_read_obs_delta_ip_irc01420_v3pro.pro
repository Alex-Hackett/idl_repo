PRO ZE_READ_OBS_DELTA_TXT_V2_TRY,model,model2d,sufix,ipdeltadatafile,rstar,np=np,ndelta1=ndelta1,nos=nos,p=p,delta1_vec=delta1_vec,obs_freq=obs_freq,ipdelta=ipdelta,lambda=lambda,reload=reload
C=299792000.
...
IF (FILE_EXIST(savedfile) eq 0) OR KEYWORD_SET(RELOAD) THEN BEGIN 
  ...
  data_ipdeltadatafile=READ_ASCII(ipdeltadatafile,data_start=0,header=header_ipdeltadatafile,count=count_ipdeltadatafile)
...
lambda_val=21645.2
ZE_IRC10420_CREATE_INTENSITYIMAGE_EPS,pa,rstar,dist,lambda_val,lambda0,lambda,np,ndelta1,nos,ipdelta,p,delta1_vec,x,y,model,model2d,sufix,dstr,circ_GridSpace_mas
lambda_val=21657.2
ZE_IRC10420_CREATE_INTENSITYIMAGE_EPS,pa,rstar,dist,lambda_val,lambda0,lambda,np,ndelta1,nos,ipdelta,p,delta1_vec,x,y,model,model2d,sufix,dstr,circ_GridSpace_mas
lambda_val=21661.2
ZE_IRC10420_CREATE_INTENSITYIMAGE_EPS,pa,rstar,dist,lambda_val,lambda0,lambda,np,ndelta1,nos,ipdelta,p,delta1_vec,x,y,model,model2d,sufix,dstr,circ_GridSpace_mas
