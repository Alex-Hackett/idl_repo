...
output='/Users/jgroh/temp/output_agc_hipparcos_mag_year_date_jd.txt'
openw,1,output
for i=0,nrec-1 do begin
printf,1,FORMAT='(F8.2,2x,F9.4,2x,F4.2,2x,I0,2x,I0,2x,I0)',jd[i]+40000.,yr_frac[i],v[i],year[i],month[i],day[i]
endfor
close,1

;output interpolated data to archive for checking
output2='/Users/jgroh/temp/output_agc_hipparcos_mag_year_date_jd_interp.txt'
openw,2,output2
for i=0.,npts-1. do begin
printf,2,FORMAT='(F8.2,2x,F9.4,2x,F4.2,2x,I0,2x,I0,2x,I0)',jdi[i]+40000.,yr_fraci[i],vi[i],yeari[i],monthi[i],dayi[i]
endfor
close,2

;output uvby  magnitudes interpolated to the epoch of the observations to archive; THAT's WHAT WE WANT FOR THE PAPER
 output3='/Users/jgroh/temp/output_agc_hipparcos_mag_year_date_jd_interp_to_spec_dates.txt'
openw,3,output3
for i=0.,nrec2-1. do begin
printf,3,FORMAT='(F8.2,2x,F9.4,2x,F4.2,2x,I0,2x,I0,2x,I0,5x,F7.2)',jd_av[i]+40000.,yr_frac_av[i],v_av[i],month_av[i],day_av[i], near2[i]
endfor
close,3
