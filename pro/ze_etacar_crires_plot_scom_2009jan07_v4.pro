;PRO ZE_ETACAR_CRIRES_PLOT_SCOM_2009JAN07_V4,scoma,swmpa,cal2070

grat_angle=1087
det=2
pos=0 ;pos 0 is on the star, by definition
xnodes=0
ynodes=0
xnodes_spec_Ext=0
ynodes_spec_ext=0
gratdet=strcompress(string(grat_angle, format='(I04)'))+'_'+strcompress(string(det, format='(I01)'))
gratdet_orig='2009jan07_'+strcompress(string(grat_angle, format='(I04)'))+'_'+strcompress(string(det, format='(I01)'))+'_offset'+strcompress(string(pos, format='(I01)')) ;in order to keep info of the original gratdet, to write the files correctly.
...
;normalize extracted spectrum in order to do better telluric subtraction
LOADCT,0
!P.Background = fsc_color('white')
!P.Color = fsc_color('black')
window,0
xyouts,0.5,0.5,'Select points to remove stellar features fro telluric subtraction', /NORMAL
...
;subtract or divide by the continuum value?
FOR I=0, sizespat -1 DO dataa2contdiv[*,i]=dataa2[*,i]/norm_line_vector[i]*0.739
