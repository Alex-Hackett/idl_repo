...
;+
;       ze_lamp_line_id
;       based on line_norm, trying ot return centroid position and centroid intensity of lines identified in a lamp spectrum to create a template
;
; Widget to interactively normalize a spectrum
;
; CALLING SEQUENCE:
;
; ze_lamp_line_id,index,flux,xnodes,ynodes,xcen,ycen
;
; INPUTS
...
;
pro ze_lamp_line_id_event,event
...
pro ze_lamp_line_id,xin,yin,xnodes,ynodes,xcen,ycen, $
...
;
  ...
  ;plotting is done using guess_lambda to help visual identification; remeber here we are to find the index of the centroids
  indexarray=findgen(n_elements(xarray))*1d0 ; vector with the index corresponding the the guess_lambda values
  xcen=xn
  for i=0, n_elements(xn)-1 DO xcen[i]=findel(xn[i],xarray)
  ycen=yn  
  ...
  centroid_find__closest_peak_window=10 ;in pixels
  centroid_fit_window=6 ;in pixels
  xcenmin=xcen - centroid_fit_window
  xcenmax=xcen + centroid_fit_window
  minx_index=WHERE(xcenmin lt 0,countmin)
  maxx_index=WHERE(xcenmax gt n_elements(xarray)-1,countmax)
  IF countmin NE 0 THEN xcenmin(where(xcenmin lt 0))=0
  IF COUNTMAX NE 0 THEN xcenmax(where(xcenmax gt n_elements(xarray)-1))=n_elements(xarray)-1
;  print,xcenmin,xcenmax
  ;find centroids automatically -- usually works well
  for i=0, n_elements(xcen) -1 DO BEGIN
  ZE_FIND_CLOSEST_PEAK_SPECTRA_1D,xcen[i]-xcenmin[i],yarray[xcenmin[i]:xcenmax[i]],xpeak
  offset=(xcen[i]-xcenmin[i])-xpeak
;  offset=0
  fit=gaussfit(indexarray[xcenmin[i]+offset:xcenmax[i]+offset],yarray[xcenmin[i]+offset:xcenmax[i]+offset],A,nterms=4)
  xcen[i]=A[1] 
  ycen[i]=A[0] 
  ENDFOR
