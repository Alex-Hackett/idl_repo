...
END
;--------------------------------------------------------------------------------------------------------------------------
PRO COMPUTE_FOURIER_TRANSFORM,dist,pgridip,freqgridip,ip,C,minindex,RAND,norm_pmas_griddedData=norm_pmas_griddedData,pmas_griddedData=pmas_griddedData,$
				ftransform=ftransform,ftransfnorm=ftransfnorm,ftxaxis=ftxaxis,ftxvector=ftxvector,ftyvector=ftyvector,$ 
				ftya=ftya,ftxb=ftxb,ftza=ftza,ftbaseline=ftbaseline,spatfreq_m_micron=spatfreq_m_micron
...
FOR i=0., RAND-1 DO BEGIN
ftza[i] = ip[i+minindex] ; intensity as a function of (p, lambda), that's the correct way for Fourier TRansform
;ftza[i] = ip[i+minindex]*pgridip[i+min]*pgridip[i+min] ; playing just to produce image p.I(p) to compare with 2D code for debuggging
ftxb[i] = C/freqgridip[i+minindex]*1E-05 ; lambda [in Angstroms]
ftya[i] = pgridip[i+minindex]/(6.96*214.08*dist) ; in [mas]
...
; Create the set of Delaunay triangles for further regular gridding of the data. The variables "triangles" and "boundaryPts" are output
; variables.
;ftxb(where(finite(ftxb,/INFINITY)))=0.01
;print,min(ftxb),max(ftxb)
print,where(finite(ftxb,/INFINITY))
...
pmas_gridSpace = [5.0, 0.2] ;first value means spacing in lambda (in angstrom), second value means spacing in p (in r/rstar)
...
FOR i=0., s[1]-2. DO BEGIN
           FOR n=0, Nbas-1 DO BEGIN
            IF KEYWORD_SET(spatfreq_m_micron) THEN ftxaxis[n]=ftbaseline[n]/(ftxvector[i]*1e-4)   ELSE BEGIN  ;spatial frequency in [m/micron], TESTING
              ftxaxis[n]=ftbaseline[n]/(206265.*ftxvector[i]*1e-10)     ;spatial frequency axis in [1/arcsec], this is correct and works ok
	          ENDELSE
	          ftransform[i,n]=0.
            FOR u=0,ys[1]-1 DO BEGIN
             k=norm_pmas_griddedData[i,u]*ftyvector[u]*BESELJ(2*3.141592*ftyvector[u]*mas_to_rad*ftbaseline[n]/(ftxvector[i]*1e-10),0)  ; if spatial frequency axis in [1/arcsec], this is correct and works ok
...
PRO PLOT_VIS_SPATFREQ,ftxvector,ftxaxis,ftransfnorm,lambda_val,spatfreq_m_micron=spatfreq_m_micron,ftransfnorm_lambda=ftransfnorm_lambda

near = Min(Abs(ftxvector - lambda_val), index)
IF (index LT 0) OR (index EQ (size(ftransfnorm))[1]) THEN BEGIN
   print,'Lambda value outside of data range. Choose a new lambda or change lstart, lend'
   REturn
END
   
ftransfnorm_lambda=REFORM(ftransfnorm[index,*])   
...
IF KEYWORD_SET(spatfreq_m_micron) THEN xtitle='spatial frequency (m/micron)' ELSE xtitle='spatial frequency (1/arcsec)'
cgPLOT,ftxaxis,ftransfnorm[index,*],xrange=[0,100.],xstyle=1,xtitle=xtitle,ytitle='Visibility',$
	title='Visibility x Sp Freq at '+lambda_str+' Angstrom'
cgPLOTs,ftxaxis,ftransfnorm[index,*],Psym=2,noclip=0
END
;--------------------------------------------------------------------------------------------------------------------------

PRO PLOT_VISSQ_SPATFREQ,ftxvector,ftxaxis,ftransfnorm,lambda_val,spatfreq_m_micron=spatfreq_m_micron,ftransfnorm_lambda=ftransfnorm_lambda
...
!Y.OMARGIN=[5,3]
!X.OMARGIN=[7,3]
WINDOW,10,xsize=600,ysize=600,retain=2

IF KEYWORD_SET(spatfreq_m_micron) THEN xtitle='spatial frequency (m/micron)' ELSE xtitle='spatial frequency (1/arcsec)'
cgPLOT,ftxaxis,(ftransfnorm[index,*])^2,xrange=[0,100.],xstyle=1,xtitle=xtitle,ytitle='Visibility squared',$
  title='Log Sq. Visibility x Sp Freq at '+lambda_str+' Angstrom',/ylog
cgPLOTs,ftxaxis,(ftransfnorm[index,*])^2,Psym=2,noclip=0
END
;--------------------------------------------------------------------------------------------------------------------------
...
;Eta Car john's model 106 (reduced Mdot)
NP=60.
NCF=63590.
...
filename_ipdata_bin='/Users/jgroh/ze_models/etacar/mod106_john/obs_groh/IP_DATA'
;filename_ipdata_bin='/Users/jgroh/ze_models/etacar/mod44_groh/obs/IP_DATA'
;filename_ipdata_bin='/Users/jgroh/ze_models/etacar/mod45_groh/obs/IP_DATA'
;filename_ipdata_bin='/Users/jgroh/ze_models/etacar/mod31_groh/obs/IP_DATA'
;filename_ipdata_bin='/Users/jgroh/ze_models/etacar/mod7_groh/obs/IP_DATA'
...
;for P2IP plots for Groh+ 2012 MNRAS paper
;lstart=800.
;lend=2000.
;lstart=2000.
;lend=3300.
...
;for analysis of PIONIER data H band
lstart=17600.
lend=17800.
;lstart=16700.
;lend=16900.
...
;lstart=15000.
;lend=19000.
;for analysis of AMBER data K band continuum
;lstart=21700.
;lend=21800.
...
index2=FINDEL(lend,lambda)
minindex=(NP)*(index-1)
maxindex=(NP)*(index2+2)
nlambda=index2+3-index
RAND=maxindex-minindex
...
;l=l+1.
;ENDFOR
use_spat_freq_in_m_micron=1
lambda_val=16720.0 ;lambda in angstroms for plots of I x radius and Visibility x spatial freq and Visibility x baseline
lambda_val=15650.0
lambda_val=17627.5
;lambda_val=18000.0
;lambda_val=21740.0 ;lambda in angstroms for plots of I x radius and Visibility x spatial freq and Visibility x baseline
;COMPUTE_LINE_PROFILE,pgridip,freqgridip,ip,dist,C,minindex,RAND,lambda_val,lineprofile=lineprofile,lambda_lineprofile=lambda_lineprofile
COMPUTE_FOURIER_TRANSFORM,dist,pgridip,freqgridip,ip,C,minindex,RAND,norm_pmas_griddedData=norm_pmas_griddedData,pmas_griddedData=pmas_griddedData,$
	  ftransform=ftransform,ftransfnorm=ftransfnorm,ftxaxis=ftxaxis,ftxvector=ftxvector,ftyvector=ftyvector,ftya=ftya,ftxb=ftxb,ftza=ftza,ftbaseline=ftbaseline,spatfreq_m_micron=use_spat_freq_in_m_micron
...
;PLOT_VIS_SPATFREQ,ftxvector,ftxaxis,ftransfnorm,lambda_val,spatfreq_m_micron=use_spat_freq_in_m_micron
PLOT_VISSQ_SPATFREQ,ftxvector,ftxaxis,ftransfnorm,lambda_val,spatfreq_m_micron=use_spat_freq_in_m_micron,ftransfnorm_lambda=ftransfnorm_lambda
