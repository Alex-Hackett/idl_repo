...
;NP=75.
;NCF=63551.
;HD45166 106copy_mod8 2D
;NP=55.
;NCF=771.

;Eta Car mod 111 John
;NP=75.
;NCF=63645.
...
;OPENR, lun, '/aux/pc244a/jgroh/ze_models/etacar_john/mod_111/obs/IP_DATA', /GET_LUN
;OPENR, lun, '/home/groh/ze_models/etacar_john/mod_97/obs/IP_DATA', /GET_LUN
OPENW,2,'/aux/pc244a/jgroh/temp/teste_IP.txt'
OPENR, lun, '/aux/pc20117a/jgroh/cmfgen_models/agcar/389/obshbetaip/IP_DATA', /GET_LUN
...
gridSpace = [0.4, 0.1] ;first value means spacing in lambda, second value means spacing in log p/R* 
gridSpace = [0.4, 0.1] ;first value means spacing in lambda, second value means spacing in  r/R*
   ; Grid the Z data, using the triangles.The variables "xvector"
   ; and "yvector" are output variables.
;[MIN(X), MIN(Y), MAX(X), MAX(Y)]
griddedData = TriGrid(x, y, z, triangles, gridSpace,[MIN(X), MIN(Y), MAX(X), 30], $
...
lineprofile=lineprofile/lineprofile[2] ;normalizing
;;   ; Display the surface.
;;
...
;tvlaser,BARPOS='no',FILENAME='/aux/pc244a/jgroh/temp/idl.ps',$
