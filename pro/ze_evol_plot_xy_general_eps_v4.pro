PRO ZE_EVOL_PLOT_XY_GENERAL_EPS_V4,x,y,xtitle,ytitle,label,$
                                ...
                                pointsx2=pointsx2,pointsy2=pointsy2,pointsx3=pointsx3,pointsy3=pointsy3,psym1=psym1,symsize1=symsize1,symcolor1=symcolor1,$
                                ...
                                factor1=factor1,factor2=factor2,factor3=factor3,factor4=factor4,factor5=factor5,factor6=factor6,factor7=factor7,factor8=factor8,factor9=factor9,factor_10=factor_10,factor_11=factor_11,$
                                plot_last=plot_last 
;working on supporting multiple vectors on the same plot. right now for vector with the same number of points.
;working OK up to 11 vectors
;v4 implements different factor, but not function for selecting symbols for SN progenitor paper
...
if N_elements(rebin_z) lt 1  THEN BEGIN ;no Z values to do color-coding
   ...
   ENDIF ELSE BEGIN; data has not been rebinned
       ...
       IF ((N_elements(x4) GT 1) AND (N_elements(y4) GT 1)) THEN cgplots,x4,y4,color=color4,noclip=0,linestyle=linestyle4
       IF ((N_elements(x5) GT 1) AND (N_elements(y5) GT 1)) THEN cgplots,x5,y5,color=color5,noclip=0,linestyle=linestyle5
       IF ((N_elements(x6) GT 1) AND (N_elements(y6) GT 1)) THEN cgplots,x6,y6,color=color6,noclip=0,linestyle=linestyle6
       IF ((N_elements(x7) GT 1) AND (N_elements(y7) GT 1)) THEN cgplots,x7,y7,color=color7,noclip=0,linestyle=linestyle7
       IF ((N_elements(x8) GT 1) AND (N_elements(y8) GT 1)) THEN cgplots,x8,y8,color=color8,noclip=0,linestyle=linestyle8   
...
IF ((N_elements(pointsx2) GT 1) AND (N_elements(pointsy2) GT 1)) THEN cgplots,pointsx2,pointsy2,color='green',psym=symcat(14),symsize=3. 
IF ((N_elements(pointsx3) GT 1) AND (N_elements(pointsy3) GT 1)) THEN cgplots,pointsx3,pointsy3,color='red',psym=symcat(15),symsize=3. 
