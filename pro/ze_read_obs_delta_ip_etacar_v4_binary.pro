...
circxvector=circxvector,circyvector=circyvector,circ_griddedData=circ_griddedData,gauss=gauss,fwhm_g,gauss_frac_wind
...
INC_BINARY=1
IF KEYWORD_SET(INC_BINARY) THEN BEGIN
;add a gaussian
gauss = PSF_GAUSSIAN( Npixel=pix, FWHM=[fwhm_g,fwhm_g], CENTROID=[513,513],/NORMAL,/DOUBLE )
;gauss=gauss/max(gauss)
...
;gauss_frac_wind=0.3 ;;fraction of gauss emission relative to the wind;
...
$MAIN CODE
PRO ZE_READ_OBS_DELTA_IP_ETACAR_V4_BINARY,dir,model,model2d,sufix,pa,cp_kband,base_min,base_max,jd_min,jd_max,compute_image=compute_image,gauss=gauss,fwhm_g,gauss_frac_wind
;v4 is a procedure ,and needs as input dir,model2d, sufix, pa
;v4 is optimize for computing images, vis x PA (24m) and CP for the etacar letter (Groh, Madura, Hillier, Owocki, Weigelt 2010). does not plot a lot of misc stuff.
...
;PA_obs=PA_obs_etacar_2005feb26
;Base_obs=Base_obs_etacar_2005feb26
...
circ_GridSpace_mas=[0.010,0.010]
...
ZE_PLOT_1DVISIBILITY_PA_BASELINE, pa_vb[i],image,vis_circxvector,vis_circyvector,circxvector,circyvector,lambda_val,profile=profilet,baseline_val=baseline_valt
vis_profile2[*,i]=profilet[0:150]
baseline_val2[*,i]=baseline_valt[0:150]
ENDFOR
...
printf,1,nbas
printf,1,npa_vb
printf,1,vis_profile2
printf,1,baseline_val2
close,1
;
...
totalfluxfimsysn=totalfluxfimsys ;not normalizing here in v4
...
ZE_IRC10420_CREATE_INTENSITYIMAGE_EPS_V3,pa,rstar,dist,lambda_val,lambda0,lambda,np,ndelta1,nos,ipdelta,p,delta1_vec,x,y,model,model2d,sufix,dstr,inc_str,circ_GridSpace_mas,$
circ_griddedData=circ_griddedData,circxvector=circxvector,circyvector=circyvector,gauss=gauss,fwhm_g,gauss_frac_wind
