...
;circ_griddedData[0:1000,000:475]=0.
...
PRO ZE_PLOT_IMAGE_I_P_DELTA_LAMBDA,rstar,dist,pa,lambda_val,lambda0,lambda,np,ndelta1,nos,ipdelta,p,delta1_vec,x,y,intens2d_lambda,$
...
IF KEYWORD_SET(vel) THEN BEGIN
title='Image at velocity '+vel_str+' km s!E-1!N, i=45!Eo!N, PA='+pa_str+'!Eo '
ENDIF ELSE BEGIN
title='Image at wavelength '+lambda_str+' Angstrom, i=45!Eo!N, PA='+pa_str+'!Eo '
ENDELSE
...
phaseminscl=-15.
phasemaxscl=15.
img=bytscl(phase,MIN=phaseminscl,MAX=phasemaxscl); byte scaling the image for display purposes with tvimage
imginv=img
;plotting in window
xsize=900
ysize=760
...
colorbar_min=phaseminscl
colorbar_max=phasemaxscl
...
dir='/Users/jgroh/ze_models/2D_models/HD316/'
model='M1'
...
rstar=522.0 ; in CMFGEN UNITS
;rstar=1500.4
;rstar=785.
;dist=5.5 ;in kpc
dist=8.0 ;in kpc
dstr=strcompress(string(dist*10., format='(I02)'))
...
pa=81.          ;position angle in degrees; by definition PA=0 towards N, and increase eastwards. USE PA significantly different than 0 (e.g.50) for spherical models
;pa=227
...
Base_obs_hd316_at_25mar11=[45.16,105.731, 85.858]
PA_obs_hd316_at_25mar11=[29.58,262.100,-72.6300] ;in degress
...
Base_obs=Base_obs_hd316_at_25mar11
PA_obs=PA_obs_hd316_at_25mar11
...
lambda_ini=21560.   ;initial lambda to do calculations 21610
lambda_fin=21785.    ;initial lambda to do calculations 21720
lambda_ini=21600.   ;initial lambda to do calculations 21610
...
lambda_sampling=1.8     ;sampling in Angstrom ; use 1
...
circxvector=circxvector,circyvector=circyvector,circ_griddedData=circ_griddedData
ZE_PLOT_IMAGE_I_P_DELTA_LAMBDA,rstar,dist,pa,lambda_val,lambda0,lambda,np,ndelta1,nos,ipdelta,p,delta1_vec,x,y,intens2d_lambda, circxvector,circyvector,circ_griddedData,imginv=imginv,/VEL
wset,8
file_output='/Users/jgroh/temp/hd316285_'+model+'_'+model2d+'_'+sufix+'_' +'i_'+ inc_Str+'_PA_'+pa_str+strcompress(string(i, format='(I04)'), /remove)+'.gif'
;WRITE_PNG, '/Users/jgroh/temp/irc10420_'+model+model2d+sufix   + '.png', TVRD(True=1)
...
circ_GridSpace_mas_vis=[0.40,0.40]
for i=0., nlambda-1 do begin
lambda_val=lambda_ini + i*lambda_sampling
lambda_vector[i]=lambda_val
...
file_output='/Users/jgroh/temp/hd316285_vis_'+model+'_'+model2d+'_'+sufix+'_'+'i_'+inc_Str+'_d'+dstr+'_PA_'+pa_str+strcompress(string(i, format='(I04)'), /remove)
image = TVREAD(FILENAME = file_output, /GIF, /NODIALOG)
wset,14
file_output='/Users/jgroh/temp/hd316285_phase_'+model+'_'+model2d+'_'+sufix+'_'+'i_'+ inc_Str+'_d'+dstr+'_PA_'+pa_str+strcompress(string(i, format='(I04)'), /remove)
image = TVREAD(FILENAME = file_output, /GIF, /NODIALOG)
endfor
...
lambda_val=21661.2 ;ONLY for baseline calculations; actually uses the last visamp for computing values...
paprof=00. ; PA in degrees
image=visamp
window,29
LOADCT,0, /SILENT
!P.Background = fsc_color('white')
...
plot, baseline_val0,profile0, xstyle=1, ystyle=1,YMARGIN=[5,5],XRANGE=[0,150],psym=1
...
plots, baseline_val45,profile45,color=FSC_COLOR('red'),noclip=0,psym=1
paprof=paprof+45. ; PA in degrees
...
plots, baseline_val90,profile90,color=FSC_COLOR('blue'),noclip=0,psym=1
paprof=paprof+45. ; PA in degrees
...
plots, baseline_val135,profile135,color=FSC_COLOR('green'),noclip=0,psym=1
paprof=paprof+45. ; PA in degrees
...
plots, baseline_val180,profile180,color=FSC_COLOR('dark green'),noclip=0,psym=1


dir_obs='/Users/jgroh/data/eso/amber/lbvs_p85/lbvs_amber_ascii/'
;file_obs='SCIENCE_wray1796_AMBER.2010-06-27T03:08:54.832-AMBER.2010-06-27T03:14:37.961_K_AVG__DIV__CALIB_hd163197_AMBER.2010-06-27T03:30:44.719-AMBER.2010-06-27T03:54:06.935_K_AVG.fits.ascii'
...
lobs=REFORM(obsdata[0,*]*10000.)-24.5 ;MANUAL SHIFTGIN
fobs=REFORM(obsdata[19,*])

vis12_ut=REFORM(obsdata[1,*])
vis23_ut=REFORM(obsdata[3,*])
vis31_ut=REFORM(obsdata[5,*])

phase12_ut=REFORM(obsdata[7,*])
phase23_ut=REFORM(obsdata[9,*])
phase31_ut=REFORM(obsdata[11,*])

cp_ut=REFORM(obsdata[13,*])
...
vis12_ut=ZE_SHIfT_SPECTRA_VEL(lobs,vis12_ut,vhelio)
vis23_ut=ZE_SHIfT_SPECTRA_VEL(lobs,vis23_ut,vhelio)
vis31_ut=ZE_SHIfT_SPECTRA_VEL(lobs,vis31_ut,vhelio)
phase12_ut=ZE_SHIfT_SPECTRA_VEL(lobs,phase12_ut,vhelio)
phase23_ut=ZE_SHIfT_SPECTRA_VEL(lobs,phase23_ut,vhelio)
phase31_ut=ZE_SHIfT_SPECTRA_VEL(lobs,phase31_ut,vhelio)
cp_ut=ZE_SHIfT_SPECTRA_VEL(lobs,cp_ut,vhelio)
...
plots,lobs,phase31_ut,color=FSC_COLOR('blue'),noclip=0
...
!P.CHARSIZE=2.0
device,/close
device,filename='/Users/jgroh/temp/vis_wavelength'+model+model2d+sufix+'_d'+dstr+'_pa'+pa_str+'.eps',/encapsulated,/color,bit=8,xsize=10*xwindowsize/ywindowsize,ysize=10,/inches
...
device,filename='/Users/jgroh/temp/spec_wavelength'+model+model2d+sufix+'_d'+dstr+'_pa'+pa_str+'.eps',/encapsulated,/color,bit=8,xsize=10*xwindowsize/ywindowsize,ysize=10,/inches
...
device,filename='/Users/jgroh/temp/phase_wavelength'+model+model2d+sufix+'_d'+dstr+'_pa'+pa_str+'.eps',/encapsulated,/color,bit=8,xsize=10*xwindowsize/ywindowsize,ysize=10,/inches
plot,lambda_vector,phase_triplet_wavelength_cnvl[0,*],yrange=[-80,80],XTITLE='Heliocentric vacuum wavelength (Angstrom)', YTITLE='Differential phase [deg]',$
/nodata,XTICKFORMAT='(I6)',xstyle=9,ystyle=1,Position=[0.16, 0.09, 0.85, 0.78*xwindowsize/ywindowsize]
plots,lobs,phase12_ut,color=FSC_COLOR('red'),noclip=0
plots,lobs,phase23_ut,color=FSC_COLOR('green'),noclip=0
plots,lobs,phase31_ut,color=FSC_COLOR('blue'),noclip=0
...
device,filename='/Users/jgroh/temp/cp_wavelength'+model+model2d+sufix+'_d'+dstr+'_pa'+pa_str+'.eps',/encapsulated,/color,bit=8,xsize=10*xwindowsize/ywindowsize,ysize=10,/inches
plot,lobs,cp_ut,yrange=[-100,100],XTITLE='Heliocentric vacuum wavelength (Angstrom)', YTITLE='F/Fc',$
/nodata,XTICKFORMAT='(I6)',xstyle=9,ystyle=1,Position=[0.16, 0.09, 0.85, 0.78*xwindowsize/ywindowsize]
plots,lobs,cp_ut,color=FSC_COLOR('black'),noclip=0
plots,lambda_vector,cp_triplet_wavelength_cnvl_sys,color=FSC_COLOR('red')
AXIS,XAXIS=1,XTICKFORMAT='(I5)',XSTYLE=1,COLOR=fsc_color('black'),XTITLE='v - vsys (km/s)',XRANGE=[ZE_LAMBDA_TO_VEL(min(lambda_vector),lambda0)-vsys,ZE_LAMBDA_TO_VEL(max(lambda_vector),lambda0)-vsys]
...
;plots using lineplot
;lineplot,lobs,fobsheln
;lineplot,lambda_vector,totalfluxfimsysn
;
;lineplot,lobs,vis12_ut
;lineplot,lambda_vector,vis_triplet_wavelength_cnvl[0,*]
;
;lineplot,lobs,vis23_ut
;lineplot,lambda_vector,vis_triplet_wavelength_cnvl[1,*]
;
;lineplot,lobs,vis31_ut
;lineplot,lambda_vector,vis_triplet_wavelength_cnvl[2,*]

lineplot,lobs,phase12_ut
lineplot,lambda_vector,phase_triplet_wavelength_cnvl[0,*]

lineplot,lobs,phase23_ut
lineplot,lambda_vector,phase_triplet_wavelength_cnvl[1,*]

lineplot,lobs,phase31_ut
lineplot,lambda_vector,phase_triplet_wavelength_cnvl[2,*]
