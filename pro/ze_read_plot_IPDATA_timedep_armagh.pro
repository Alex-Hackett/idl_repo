...
pmas_gridSpace = [0.5, 0.2] ;first value means spacing in lambda (in angstrom), second value means spacing in p (in r/rstar)
...
PRO PLOT_IMAGE_P2IP_LAMBDA_R,ip,pgridip,freqgridip,C,min,max,RAND,rstar
...
FOR i=0., RAND-1 DO BEGIN
ftza[i] = ip[i+min];/max(ip[i+min]);*pgridip[i+min];*pgridip[i+min] ; intensity as a function of (p, lambda)
ftxb[i] = C/freqgridip[i+min]*1E-05 ; lambda [in Angstroms]
...
img=bytscl(p2ip_griddedData,MAX=max(p2ip_griddedData)); byte scaling the image for display purposes with tvimage
;imginv=255b-img ;invert img
imginv=img
;plotting in window
window,8,xsize=900,ysize=900,retain=2
...
;norm_pmas_griddedData=alog10(norm_pmas_griddedData)
;denconbetatrunc = [0.00, max(loggriddedData)*.2, max(loggriddedData)*.4, max(loggriddedData)*.6, max(loggriddedData)*.8,max(loggriddedData)]
colorbar, COLOR=fsc_color('black'),TICKNAMES=denconbetatrunc, FORMAT='(F5.1)', DIVISIONS=5, /VERTICAL, /RIGHT,$
POSITION=[0.86, 0.08, 0.88, 0.95]
LOADCT,0
AXIS,XAXIS=0,XTICKFORMAT='(F7.1)',XSTYLE=1,COLOR=0,XTITLE='',XRANGE=[min(ftxvector),max(ftxvector)],xcharsize=2
AXIS,YAXIS=0,YSTYLE=1,COLOR=0,ycharsize=2
...
PRO PLOT_IMAGE_P2IP_NORM_LAMBDA_R,ip,pgridip,freqgridip,C,min,max,RAND,lambda_lineprofile,lineprofile,rstar
!P.Background = fsc_color('white')
...
yrange=[min(ftyvector),max(ftyvector)],xstyle=1,ystyle=1, xtitle='velocity (km/s)', $
ytitle='distance  (r/rstar)', /NODATA, Position=[0.07, 0.08, 0.85, 0.95],ycharsize=2,xcharsize=2
colors = GetColor(/Load)
!P.Background = colors.white
!P.Color = colors.black
...
;tvlaser,BARPOS='no',FILENAME='/home/groh/idl.ps',$
;  /NoPrint,XDIM=600,YDIM=600
oplot,lambda_lineprofile,3+lineprofile/lineprofile[0]
END
;--------------------------------------------------------------------------------------------------------------------------
PRO PLOT_IMAGE_P2IP_NORM_VEL_R,lambda_val,ip,pgridip,freqgridip,C,min,max,RAND,lambda_lineprofile,lineprofile,rstar,norm_p2ip_griddedData,xvector_norm_p2ip_griddedData, yvector_norm_p2ip_griddedData
!P.Background = fsc_color('white')
;define axis x,y,z
ftza=dblarr(RAND)
ftxb=ftza
...
;recommended values FOR ETACR :0.01,30 (greater than 30 and lower than 0.01 does not change anything FOR ETACAR, see xmgrace plot)
; Grid the Z data, using the triangles.The variables "xvector" and "yvector" are output variables. Set grid space by:
p2ip_gridSpace = [0.5, sp] ;first value means spacing in velocity (km/s), second value means spacing in p (in mas)
p2ip_griddedData = TRIGRID(ftxb, ftya, ftza, fttriangles, p2ip_gridSpace,[MIN(ftxb), 0., MAX(ftxb), 15.], XGrid=xvector_norm_p2ip_griddedData, YGrid=yvector_norm_p2ip_griddedData)

s=size(xvector_norm_p2ip_griddedData)
ys=size(yvector_norm_p2ip_griddedData)
...
norm_p2ip_griddedData=alog10(1.+norm_p2ip_griddedData)
img=bytscl(norm_p2ip_griddedData,MAX=max(norm_p2ip_griddedData)/2.,MIN=0.); byte scaling the image for display purposes with tvimage
;imginv=255b-img ;invert img
imginv=img
;plotting in window
...
!P.Background = colors.white
!P.Color = colors.black
plot, xvector_norm_p2ip_griddedData, yvector_norm_p2ip_griddedData, XTICKFORMAT='(F7.1)', xrange=[min(xvector_norm_p2ip_griddedData),max(xvector_norm_p2ip_griddedData)], $
yrange=[min(yvector_norm_p2ip_griddedData),max(yvector_norm_p2ip_griddedData)],xstyle=1,ystyle=1, xtitle='Velocity (km/s)', $
ytitle='impact parameter  (r/rstar)', /NODATA, Position=[0.07, 0.08, 0.85, 0.95],ycharsize=2,xcharsize=2
colors = GetColor(/Load)
!P.Background = colors.white
!P.Color = colors.black
LOADCT, 13
...
!P.Background = colors.white
!P.Color = colors.black
AXIS,XAXIS=0,XTICKFORMAT='(F7.1)',XSTYLE=1,COLOR=0,XTITLE='',XRANGE=[min(xvector_norm_p2ip_griddedData),max(xvector_norm_p2ip_griddedData)],xcharsize=2
AXIS,YAXIS=0,YSTYLE=1,COLOR=0,ycharsize=2
xyouts,830,870,TEXTOIDL('p I(p)'),/DEVICE,color=fsc_color('black')
;xyouts,0.3,0.3,'test',/DEVICE,charsize=5
;;colorbar, COLOR=0, YTickV=[0,28,85,142,199,255],Range=[0,255], $
...
;tvlaser,BARPOS='no',FILENAME='/home/groh/idl.ps',$
;  /NoPrint,XDIM=600,YDIM=600
oplot,ZE_LAMBDA_TO_VEL(lambda_lineprofile,lambda_val),3+lineprofile/lineprofile[0],color=fsc_color('red')
END
;--------------------------------------------------------------------------------------------------------------------------
;--------------------------------------------------------------------------------------------------------------------------
PRO PLOT_IMAGE_P2IP_CONTSUB_VEL_R,lambda_val,ip,pgridip,freqgridip,C,min,max,RAND,lambda_lineprofile,lineprofile,rstar
!P.Background = fsc_color('white')
;define axis x,y,z
ftza=dblarr(RAND)
ftxb=ftza
...
;trying to generate a continuum subtracted p*I(p) image)
FOR i=0,N_elements(ftxvector)-1 DO BEGIN
 FOR k=0,N_elements(ftyvector)-1 DO BEGIN 
  norm_p2ip_griddedData[i,k]=p2ip_griddedData[i,k]-(p2ip_griddedData[1,k]) ;normalized by the total continuum flux! MUCH BETTER TO INTERPRET!
ENDFOR
ENDFOR
...
imginv=img
;plotting in window
window,22,xsize=900,ysize=900,retain=0
LOADCT,0
colors = GetColor(/Load)
!P.Background = colors.white
!P.Color = colors.black
...
;timedep armagh 6 obsip (vturb=20 km/s)
;NP=90.
;NCF=56310.0

;timedep armagh 6 obsip_v10 (vturb = 10km/s)
;NP=90.
;NCF=93639.0

;timedep armagh 14 obsip_halpha_v10 (vturb = 10km/s)
;NP=90.
;NCF=2783.0

;timedep armagh 16 obsip_halpha_v10 (vturb = 10km/s)
NP=85.
NCF=2786.0

;timedep armagh 22 obsip_halpha_v10 (vturb = 10km/s)
;NP=90.
;NCF=2784.0

;timedep armagh 48 obsip_halpha_v10 (vturb = 10km/s)
;NP=105
;NCF=2786.0
...
filename_ipdata_txt='/Users/jgroh/temp/teste_IP.txt'
...
filename_ipdata_bin='/Users/jgroh/ze_models/timedep_armagh/6/obsip_v10/IP_DATA'
filename_ipdata_bin='/Users/jgroh/ze_models/timedep_armagh/16/obsip_halpha_v10/IP_DATA'
;filename_ipdata_bin='/Users/jgroh/ze_models/timedep_armagh/22/obsip_halpha_v10/IP_DATA'
;filename_ipdata_bin='/Users/jgroh/ze_models/timedep_armagh/48/obsip_halpha_v10/IP_DATA'
...
lambda_val=6564.61 ;lambda in angstroms in VACUUM for plots of I x radius and Visibility x spatial freq and Visibility x baseline
...
PLOT_IMAGE_P2IP_LAMBDA_R,ip,pgridip,freqgridip,C,min,max,RAND,rstar
;PLOT_IMAGE_P2IP_NORM_LAMBDA_R,ip,pgridip,freqgridip,C,min,max,RAND,lambda_lineprofile,lineprofile,rstar
PLOT_IMAGE_P2IP_NORM_VEL_R,lambda_val,ip,pgridip,freqgridip,C,min,max,RAND,lambda_lineprofile,lineprofile,rstar,norm_p2ip_griddedData,xvector_norm_p2ip_griddedData,yvector_norm_p2ip_griddedData
PLOT_IMAGE_P2IP_CONTSUB_VEL_R,lambda_val,ip,pgridip,freqgridip,C,min,max,RAND,lambda_lineprofile,lineprofile,rstar

lineplot,xvector_norm_p2ip_griddedData,REFORM(norm_p2ip_griddedData[*,200])
;lineplot,yvector_norm_p2ip_griddedData,REFORM(norm_p2ip_griddedData[200,*])
