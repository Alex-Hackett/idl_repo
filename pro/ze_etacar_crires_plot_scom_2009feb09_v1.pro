;PRO ZE_ETACAR_CRIRES_PLOT_SCOM_2009FEB09_V1,scoma,swmpa,cal2070
...
swmpa=dirsci+'CR_SWMA_352642_2009-02-09T07_34_01.072_DIT1_2070.4nm.fits' ;SWMA files look better
...
swmpa=dirsci+'CR_SWMA_352642_2009-02-09T07_46_56.501_DIT1_2076.6nm.fits' ;SWMA files look better
...
;scoma=dircriresrun+'GEN_CALIB/raw/STD/CRIRE.2009-02-09T05_36_20.910.fits'
;scoma=dircriresrun+'GEN_CALIB/raw/STD/CRIRE.2009-02-09T05_35_45.027.fits'
swmpa=dirsci+'CR_SWMP_352647_2009-02-09T06_12_38.858_DIT1_1087.3nm.fits' ;SWMA files look better
...
swmpa=dirsci+'CR_SWMA_352645_2009-02-09T07_06_35.831_DIT1_2150.0nm.fits' ;SWMA files look better
...
USE_OLD_CAL_TEL:
polycal_lamp=dblarr(3)
polycal_lamp[0]=yfit[0]
polycal_lamp[1]=yfit[1]
polycal_lamp[2]=yfit[2]
for i=0., sizespat - 1 do begin
polycal_master[i,0]=yfit[0]
polycal_master[i,1]=yfit[1]
polycal_master[i,2]=yfit[2]
...
fluxteli=cspline(lambdatel,f2canorm,lambda_newcalt)
fluxteli=ZE_SHIFT_SPECTRA_VEL(lambda_newcalt,fluxteli,0.0)
;fwhm=ABS(6.017 -0.4298*i)
;fwhm=ABS(9.017 -0.4298*i)
;print,fwhm
;fluxteli=cnvlgauss(fluxteli,fwhm=fwhm)
fluxteli_2d[*,i]=fluxteli
endfor
...
FOR j=0, sizespat - 1 DO BEGIN 
  i=0.
  resid=0.
  ;added for geb 09 JHG April 9, 19 10 pm, multiplying the tel spectrum  by the normalized base.
  
  FOR scl=scl_min, scl_max, scl_step DO BEGIN
    resid=0.
;    scidiv=dataa2[*,j]/((((fluxteli_2d[*,j]*norm_spec_raw_extn)-1.)*scl)+1)
;    resid=TOTAL(SQRT(ABS(norm_line_vector[j]-scidiv)^2))
    ftel_scl=(((fluxteli_2d[*,j]-1.)*scl)+1.)*norm_spec_raw_extn
  ...
  ;print,scl_optimum[j]
;added aPril 7th JHG for better subtractions of 1090 lines
  ;scl_optimum[j]=2.
...
;FOR I=0, sizespat -1 DO dataa2[*,i]=dataa2[*,i]-(norm_line_vector[i]*spec_extn)
...
ps_filename='/Users/jgroh/temp/etc_crires'+gratdet+'sub.eps'
set_plot,'ps'
