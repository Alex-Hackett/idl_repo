...
;OPENR, lun, '/home/groh/ze_models/HD316285/314_rosella/IP_DATA', /GET_LUN
OPENR, lun, '/home/groh/ze_models/agcar/var_191regrid223_mdot8_222/ipdata_h4101/IP_DATA', /GET_LUN
...
y[i] = pgridip[i+min]/(6.96*214.08*dist)
endfor
...
gridSpace = [0.1, 0.1] ;first value means spacing in lambda, second value means spacing in mas 

   ; Grid the Z data, using the triangles.The variables "xvector"
   ; and "yvector" are output variables.
;[MIN(X), MIN(Y), MAX(X), MAX(Y)]
griddedData = TriGrid(x, y, z, triangles, gridSpace,[MIN(X), MIN(Y), MAX(X), 10.], $
...
;plotting in window
window,5,xsize=900,ysize=900,retain=2
...
ytitle='distance  (mas)', /NODATA, Position=[0.07, 0.08, 0.85, 0.95],xcharsize=2,ycharsize=2
