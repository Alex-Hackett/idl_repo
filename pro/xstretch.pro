...
;
; Copyright 1996-2008 Fanning Software Consulting.
;
...
; display window.
   ...
   CASE info.type OF
      ...
      'EQUALIZATION': BEGIN
         histdata = Histogram(Hist_Equal(*info.image), Binsize=binsize, $
            OMIN=omin, OMAX=omax)/Float(N_Elements(*info.image))
         imageTitle = 'Equalized Image Value'
         END
      'SQUARE ROOT': BEGIN
         histdata = Histogram(SQRT(*info.image), Binsize=binsize, $
            OMIN=omin, OMAX=omax)/Float(N_Elements(*info.image))
         imageTitle = 'Square Root of Image Value'
         END

      ELSE: BEGIN
         histdata = Histogram(*info.image, Binsize=binsize, OMIN=omin, OMAX=omax)/Float(N_Elements(*info.image))
   ...
   IF info.type NE selection THEN BEGIN
      CASE StrUpCase(selection) OF
         ...
         'LINEAR 2%': BEGIN
            ...
            range = maxr - minr
            IF Size(*info.image, /TName) EQ 'BYTE' THEN binsize = 1.0 ELSE binsize = range / 300.
            h = Histogram(*info.image, BINSIZE=binsize, OMIN=omin, OMAX=omax)
