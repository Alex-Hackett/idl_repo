;PRO ZE_ETACAR_CRIRES_PLOT_SCOM_2009FEB09_V3,scoma,swmpa,cal2070
...
;    print,velshift,vel_resid
;    i=i+1
;  ENDFOR
;  vel_resid_zero=WHERE(vel_resid_master eq 0., count)
;  if count gt 0. THEN vel_resid_master(vel_resid_zero)=10000000.
...
;  
;ENDFOR
;calculate the best velocity shift between telluric and science calibrations, WHEN different lamps were used. ASSUMES SHIFT IS GIVEN BY THE CENTRAL LINE (ON STAR), WORKING FINE
velshift_min=-1.6
velshift_max=-0.3
velshift_step=0.1
velshift_size=(velshift_max-velshift_min)/velshift_step + 1.
...
ftemp1=temp1
FOR I=0., velshift_size -1 DO velshift_vector[i]=velshift_min+ (velshift_step*i)
vel_resid_master=dblarr(velshift_size)
vel_min_resid_vector=lonarr(sizespat)
temp1[*]=lambda_newcal_tel[*,row]
ftemp1[*]=fluxteli_2d[*,row]
help,vel_resid_master
i=0
FOR velshift=velshift_min, velshift_max, velshift_step DO BEGIN
    resid=0.
    ftel_velshift=ZE_SHIFT_SPECTRA_VEL(temp1,ftemp1,velshift)*norm_spec_raw_extn
    vel_resid=TOTAL(SQRT(ABS((dataa2[*,row]/norm_line_vector[row])-ftel_velshift)^2.))
    print,i
    vel_resid_master[i]=vel_resid
    print,velshift,vel_resid
    i=i+1
ENDFOR
  vel_resid_zero=WHERE(vel_resid_master eq 0., count)
  if count gt 0. THEN vel_resid_master(vel_resid_zero)=10000000.
  velshift_temp=MIN(vel_resid_master, velshift_min_index)
  velshift_optimum= velshift_vector[velshift_min_index]
FOR I=0, sizespat -1 DO BEGIN 
  temp1[*]=lambda_newcal_tel[*,i]
  ftemp1[*]=fluxteli_2d[*,i] 
  ftel_shift_t=ZE_SHIFT_SPECTRA_VEL(temp1,ftemp1,velshift_optimum)
  fluxteli_2d[*,i]=ftel_shift_t
ENDFOR
GOTO,SCALE_TEL_FWHM

SCALE_TEL_FWHM:
;TESTING computes telluric spectrum template from the central row of the data
tel_template_from_science=dataa2[*,row]/norm_line_vector[row]/norm_spec_raw_extn
...
FOR j=0, sizespat - 1 DO BEGIN 
 ...
 ;     print,i,k,j,scl,fwhm_val,resid
  ...
  ;dataa2[*,j]=dataa2[*,j]/tel_template_from_science
