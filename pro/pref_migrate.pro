; $Id: pref_migrate.pro,v 1.15 2005/02/01 20:24:31 scottm Exp $
;
...
pro pref_migrate_mark
  ...
  dir =  APP_USER_DIR_QUERY('rsi', 'pref', /RESTRICT_IDL_RELEASE, $
...
function pref_migrate_ok, num_potential
  ...
  if (err ne 0) then begin
    CATCH,/CANCEL
    MESSAGE, /continue, /noname, 'This appears to be the first time you are running this version of IDL. IDL would like to run the PREF_MIGRATE utility to help you copy your IDL preference settings from another version of IDL, but cannot because you do not seem to have an X11 graphics server available. Please consider running PREF_MIGRATE manually at some other time when you have a server available.'
    return, 0
  endif
...
pro pref_migrate_not_possible, name
  ...
  if (err ne 0) then begin
    CATCH,/CANCEL
    MESSAGE, /continue, /noname, 'PREF_MIGRATE is unable to run because you do not seem to have an X11 graphics server available. Please consider running PREF_MIGRATE manually at some other time when you have a server available.'
    return
  endif
...
pro pref_migrate, STARTUP=startup, MACRO=macro, PREFERENCE=pref
  ...
  donor_dirs = APP_USER_DIR_QUERY('rsi', 'pref', /RESTRICT_IDL_RELEASE, $
				  /RESTRICT_FAMILY, /QUERY_IDL_RELEASE, $
				  /EXCLUDE_CURRENT, count=c)
