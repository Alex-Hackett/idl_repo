PRO ZE_CMFGEN_SPEC_FIND_AVAILABLE_TIMESTEPS,modelstr,timestep_available_obs_fin_sort_match,allobsfinfiles_sort_match,allobscontfiles_sort_match,dirmod=dirmod
...
for i=0, n_elements(allobsfinfiles) -1 do timestep_available_obs_fin[i]=long(strmid(allobsfinfiles[i],strpos(allobsfinfiles[i],modelstr)+15,6))
...
for i=0, n_elements(allobscontfiles) -1 do timestep_available_obs_cont[i]=long(strmid(allobscontfiles[i],strpos(allobscontfiles[i],modelstr)+15,6))
