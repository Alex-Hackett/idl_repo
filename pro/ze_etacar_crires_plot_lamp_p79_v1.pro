;PRO ZE_ETACAR_CRIRES_PLOT_LAMP_P79_V1,scoma,swmpa,cal2070
;implementing extraction at multiple locations in the detector, and tracing a polynomium 
grat_angle=1087
det=2
xnodes=0
ynodes=0

gratdet='p79_'+strcompress(string(grat_angle, format='(I04)'))+'_'+strcompress(string(det, format='(I01)'))
;gratdet='2009feb09_tst_'+strcompress(string(grat_angle, format='(I04)'))+'_'+strcompress(string(det, format='(I01)'))
...
dirrun='/Users/jgroh/data/eso_vlt/crires/Etacar/P79/'
dirgencalib=dirrun+'GEN_CALIB/' ;for all grats and dets
bpm_file=dirgencalib+'proc/FLAT/CR_PBPM_070418A_3218.6nm.fits'
...
1030: BEGIN
dirsci=dirrun+'270196/sci_proc/'
dircal=dirrun+'270204/cal_proc/'
scoma=dirgencalib+'raw/ARC/CRIRE.2007-04-19T04_37_13.407.fits'
swmpa=dirsci+'CR_SWMA_270196_2007-04-19T04_19_30.235_DIT10_1030.9nm.fits' ;SWMA files look better
lampfile='/Users/jgroh/espectros/thar_kerber_cut_vac.txt'
flatfile=dirgencalib+'/proc/FLAT/CR_PFLT_070418A_1030.9nm.fits'
row=255
...
END
 ...
 2070: BEGIN
dirsci=dirrun+'361950/sci_proc/'
dircal=dirrun+'361952/cal_proc/'
...
lambda0=2.05869
 ...
 1087: BEGIN
dirsci=dirrun+'270210/sci_proc/'
dircal=dirrun+'270202/cal_proc/' ;no cal obs were done!
scoma=dirgencalib+'raw/ARC/CRIRE.2009-04-03T01_40_40.604.fits'
swmpa=dirsci+'CR_SWMA_361944_2009-04-03T01_27_00.321_DIT1_1087.3nm.fits' ;SWMA files look better
cal2070=dircal+'CR_PEXT_070418A_DIT10_1087.3nm.fits'
lampfile='/Users/jgroh/espectros/thar_kerber_cut_vac.txt'
flatfile=dirgencalib+'/proc/FLAT/CR_PFLT_070418A_1087.3nm.fits'
row=255
lambda0=1.08333
    END

;actually 1087, but for lamp after calibrator star; convention is 1086
 1086: BEGIN
dirsci=dirrun+'361944/sci_proc/'
dircal=dirrun+'361942/cal_proc/' ;no cal obs were done!
scoma=dirgencalib+'raw/ARC/CRIRE.2009-04-03T02_47_42.190.fits'
swmpa=dirsci+'CR_SWMA_361944_2009-04-03T01_27_00.321_DIT1_1087.3nm.fits' ;SWMA files look better
...
dirsci=dirrun+'361944/sci_proc/'
dircal=dirrun+'361942/cal_proc/' ;no cal obs were done!
scoma=dirgencalib+'raw/ARC/CRIRE.2009-04-03T02_08_13.539.fits'
swmpa=dirsci+'CR_SWMA_361944_2009-04-03T01_55_45.352_DIT1_1090.4nm.fits' ;SWMA files look better
...
lambda0=1.08333
 ...
 2150: BEGIN
dirsci=dirrun+'361947/sci_proc/'
dircal=dirrun+'361940/cal_proc/'
...
spec_ext_array=dblarr(sizelam,next)
apradius=3
for i=0, next-1 do begin
center_ext=ypos_vec[i]-row   ; offset relative to the central row
ZE_EXTRACT_CRIRES_SPEC_V1,dataa2,sizelam,row,center_ext,apradius,spec_ext=spec_ext
spec_ext_array[*,i]=spec_ext
endfor
...
CASE grat_angle of
 ...
 1030: BEGIN
        spec_ext=spec_ext+1
        spec_ext_array=spec_ext_array+1.
        ZE_ETACAR_CRIRES_WAVECAL_LAMP_V2,gratdet,lampfile,index,spec_ext_array,center_ext,swmpa2,tharlinescut,next
      ;  ZE_ETACAR_CRIRES_WAVECAL_LAMP_V1,gratdet,lampfile,index,spec_ext_array[*,i],center_ext+row,swmpa2,tharlinescut
        fitlamval=dblarr(sizelam,next)
        ndegfit=2
        fitvalues_allext=dblarr(sizelam,next)
        lamvalues_allpix=dblarr(sizelam,sizespat)
        res_toline_256_alllam=dblarr(sizelam,sizespat)
        fitcoef_alllam=dblarr(sizelam,ndegfit+1)        
        FOR I=0, next -1 DO BEGIN
          restore,'/Users/jgroh/espectros/etc_poly_wavecal_'+gratdet+'pos'+strcompress(string(i, format='(I02)'))+'_line_auto_lamp.sav'
          FOR j=0, sizelam -1 DO fitlamval[j,i]=yfit[0]+index[j]*yfit[1]+yfit[2]*index[j]*index[j]
        ENDFOR 
        
        FOR I=0,sizelam-1 DO BEGIN
          fitvalues = poly_fit2(ypos_vec,fitlamval[i,*] , ndegfit)
          fitcoef = poly_fit(ypos_vec,fitlamval[i,*] , ndegfit)
          fitvalues_allext[i,*]=(transpose(fitvalues))[*]      
          fitcoef_alllam[i,*]=(transpose(fitcoef))[*]
            for j=0, sizespat -1 do lamvalues_allpix[i,j]=fitcoef[0]+indexspat[j]*fitcoef[1]+fitcoef[2]*indexspat[j]*indexspat[j]
            for j=0, sizespat -1 do res_toline_256_alllam[i,j]=lamvalues_allpix[i,j]-lamvalues_allpix[i,256]
        ENDFOR
        save,lamvalues_Allpix,filename='/Users/jgroh/espectros/etc_poly_wavecal_'+gratdet+'_line_auto_lamp_2d.sav'
       END  
