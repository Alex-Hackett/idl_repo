...
PRO ZE_PLOT_IMAGE_RTAU_LAMBDA_P_EPS,ip,pgridip,freqgridip,NP,lambda,lstart,lend,rstar,p2ip_griddedData,ftxvector,ftyvector
...
ftza[i] = ip[i+min]/1.49e3 ; in AU, R (tau=2/3) as a function of (p, lambda)
...
p2ip_gridSpace = [10., 1.5] ;first value means spacing in lambda (in angstrom), second value means spacing in p (in mas)
...
device,filename='/Users/jgroh/temp/img_rtau_'+lstart_str+'_'+lend_str+'.eps',encapsulated=1,/color,bit=8,xsize=10*xwindowsize/ywindowsize,ysize=10,/inches
...
AXIS,XAXIS=0,XTICKFORMAT='(A2)',XSTYLE=1,COLOR=fsc_color('white'),XRANGE=[-max(ftxvector),max(ftxvector)],xcharsize=xc
...
plots,[xmin,xmax],[23.4,23.4],linestyle=2,noclip=0,thick=tb+3.5,color=colorm1
plots,[xmin,xmax],[10.0,10.0],linestyle=2,noclip=0,thick=tb+3.5,color=colorm2
plots,[xmin,xmax],[5.0,5.0],linestyle=2,noclip=0,thick=tb+3.5,color=colorm3
plots,[xmin,xmax],[1.5,1.5],linestyle=2,noclip=0,thick=tb+3.5,color=colorm4
