...
;*CATEGORY:
;
;*PURPOSE:  To convert an answer to a yes/no question into an integer ( 1 OR 0).
; 
;*CALLING SEQUENCE: 
;    YESNO,IN
; 
...
;     Dec  29, 1990  JKF/ACC     converted for use by IDL V2 (GHRS DAF).
;-
;***********************************************************************
PRO YESNO,IN
;
...
;    type=1 for string, type>1 for number 
;
      TYPE=SIZE(IN)
      IF TYPE(0) EQ 0 THEN TYPE= vmscode(TYPE(1)) ELSE BEGIN
         IN=FIX(IN(0))
         TYPE=vmscode(4)
      END
;
;
      IF TYPE EQ 1 THEN BEGIN     ; if input is string
ENT:     A=FIX(BYTE(IN))
         SA = SIZE(A)
