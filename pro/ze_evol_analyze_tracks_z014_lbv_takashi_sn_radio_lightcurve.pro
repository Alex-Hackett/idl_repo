;PRO ZE_EVOL_ANALYZE_TRACKS_Z014_LBV_TAKASHI_SN_RADIO_LIGHTCURVE
...
if N_elements(rebin_z) lt 1  THEN BEGIN ;no Z values to do color-coding
   if keyword_set(rebin) THEN BEGIN;datah as been rebinned
       ...
       cgplots,xr,yr,color='red',noclip=0,psym=psym1,symsize=symsize1,symcolor=symcolor1
...
xyouts,0.27,0.26,'Z=0.014',/NORMAL,color=fsc_color('black')
xyouts,0.27,0.22,TEXTOIDL('v_{rot}/v_{crit}=0.4'),/NORMAL,color=fsc_color('black')
