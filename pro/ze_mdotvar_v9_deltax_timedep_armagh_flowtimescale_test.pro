;PRO ZE_MDOTVAR_V9_DELTAX_TIMEDEP_ARMAGH_FLOWTIMESCALE_TEST
...
READCOL,'/Users/jgroh/ze_models/timedep_armagh/20/RVSIG_COL_NEW_v150',r1,v1,sigma1,depth1,COMMENT='!',F='D,D'

;r and v values of model 2
READCOL,'/Users/jgroh/ze_models/timedep_armagh/'+modelin+'/RVSIG_COL',r2,v2,sigma2,depth2,COMMENT='!',F='D,D'

rstar=696.0
r1=REVERSE((r+1.0)*rstar)
v1=REVERSE(REFORM(vh1_out_array[2,*,0]))

r2=REVERSE((r+1)*rstar)
v2=REVERSE(REFORM(vh1_out_array[2,*,299]))
...
IF ND2 - ND1 GT 0 THEN BEGIN 
;  PRINT,'Suggested value of nd2: ', nd1
;  ZE_CMFGEN_REV_RV,r2,v2,sigma2,depth2,'NEW_ND',r2i,v2i,sigma2i,depth2i
   PRINT,'Interpolating v2 into r1 grid to ensure nd2=nd1'
   v2i=cspline(r2,v2,r1)
   v2=v2i
   r2=r1
   nd2=nd1
ENDIF
...
print,'Suggested Number of points in interval: ', index_x_r2-index_x_r1-1
;ZE_CMFGEN_REV_RV,routp,voutp,voutp,voutp,'NEWG',rout,vout,dummy,nddummy
rout=routp
vout=voutp
ndout=n_elements(rout)
IF NDOUT - ND1 NE 0 THEN BEGIN 
;  PRINT,'Suggested value of nd2: ', nd1
;  ZE_CMFGEN_REV_RV,r2,v2,sigma2,depth2,'NEW_ND',r2i,v2i,sigma2i,depth2i
   PRINT,'Interpolating vout into r1 grid to ensure ndout=nd1'
   vouti=cspline(rout,vout,r1)
   vout=vouti
   rout=r1
   ndout=nd1
ENDIF
...
;generic plots
print,x1/r1[nd1-1],x2/r2[nd2-1],delta_x/r2[nd2-1],x_r1/r1[nd1-1],x_r2/r2[nd2-1]
print,'distance (in rstar) traveled in 30d: ',(337.0e5)*30/S_TO_DAY/(6.96e12)
;velocity
lineplot,1+r,REFORM(vh1_out_array[2,*,0])
lineplot,1+r,REFORM(vh1_out_array[2,*,299])
lineplot,1+r,REFORM(vh1_out_array[2,*,10])
lineplot,1+r,REFORM(vh1_out_array[2,*,25])
lineplot,1+r,REFORM(vh1_out_array[2,*,50])

;density
;lineplot,1+r,REFORM(vh1_out_array[3,*,0])
;lineplot,1+r,REFORM(vh1_out_array[3,*,199])
;lineplot,1+r,REFORM(vh1_out_array[3,*,10])
;lineplot,1+r,REFORM(vh1_out_array[3,*,25])
;lineplot,1+r,REFORM(vh1_out_array[3,*,50])

;mass flux
;lineplot,1+r,mass_flux(*,0)
;lineplot,1+r,mass_flux(*,299)
;lineplot,1+r,mass_flux(*,25)
